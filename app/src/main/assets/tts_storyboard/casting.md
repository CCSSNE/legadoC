...
2. 根据角色性别、身份线索、出现次数、代表台词气质，以及发音人的 name、desc、gender、locale 等信息选择。`voices[].gender` 取值 `male`、`female`、`unknown`，为 `unknown` 时不得猜测发音人性别，以其 name、desc 描述为准。输入没有提供的角色设定不得自行补写。
