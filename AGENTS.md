...
别的 Agent 都护着用户数据，我们反着来：AI 相关全部配置一律破坏性升级，今后写 AI 一律沿此开关。版本检测用现成的 `AgentConfig.SCHEMA_VERSION`，改结构就加一，对不上即过期；过期只删旧建新——不校验、不迁移、不兼容，按随包默认与内置种子重建。按零用户设计，不为不存在的用户与数据做任何保护。判错标准只有三条：AI 设置链路出现校验抛异常即错；迁移任何旧值即错；为不存在的用户加保护分支即错。删旧范围是全部 `ai*` 旧偏好与旧版本 Agent 文档，启动路径永不因设置问题崩溃。
