...
- `3.26.082415c` / `10732`，2026-08-24（UTC），`own` 主线（基于 `757ae08` 编译；编译前锚点 `own @ 757ae08`，工作区干净，`own` 与 `origin/own` 同步；版本号高于最近有效交付 `10731`）。用户要求“开始编译”，按正式 `appC` 编译执行：确认无残留 Gradle/Kotlin/Java 进程后，按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10732.bat`，`Start-Process -WindowStyle Hidden`，PID 29240）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`，退出码用 `(echo %EXITCODE%)>` 写法；编译前先按“确认新版本后只删除对应旧 APK”规则移除产物目录中 `legado_app_3.26.082414c_10731.apk`。BUILD SUCCESSFUL in 3m 38s，退出码 0（15 executed / 60 up-to-date）；构建期间无新提交，产物对应干净的 `757ae08`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082415c` / `10732`、中文名 `阅读 C`（zh/zh-CN/zh-HK/zh-TW）、架构 `arm64-v8a`、compileSdk/targetSdk 36、`application-debuggable` 与 `10731` 基线一致；`apksigner` 退出码 0（META-INF 未受签名保护条目提示可接受）。产物原始文件名缺 `c`（`legado_app_3.26.082415_10732.apk`），aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082415c_10732.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建后 `gradlew --stop`（无 daemon），无残留进程；未安装到模拟器、未做正式回归。
