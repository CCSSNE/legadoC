...
- `3.26.082707c` / `10749`，2026-08-27（UTC），`own` 主线（编译前锚点 `own @ 206b578`，`own` 与 `origin/own` 同步；基于基线 `10748` 后新增提交 `1ef7261`（修复双击跨页段落未回到真正段首）、`206b578`（朗读起点回退时开跟随地板，修复从本页读跳回上一页）。用户要求“编译”，按正式 `appC` 编译执行：22 个 stat 缓存脏标记文件经 blob 对比确认与 HEAD 完全一致，无残留构建进程，按内存压力规则冷编译 `:app:assembleAppC`。后台 `.bat`（`build_10749.bat`，回读校验 14 行后 `Start-Process -WindowStyle Hidden`）日志与退出码写入 `D:\AI\audio\build-tmp`。编译一次通过：BUILD SUCCESSFUL in 2m 24s（76 actionable tasks: 14 executed, 62 up-to-date），err.log 为空，退出码 0；构建期间无新提交。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082707c` / `10749`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`，内部元数据正确，按规则改名为 `legado_app_3.26.082707c_10749.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`；删除同目录旧 `10748` APK。本次 copyRoomSchemas 未生成新的未跟踪 schemas。构建后 `gradlew --stop`（无 daemon）、无残留构建进程；未安装到模拟器、未做正式回归。
