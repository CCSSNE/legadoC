...
- 2026-09-03：自适应图标（`mipmap-anydpi-v26`）可视区仅为 108dp 画布中央的 72dp（66.7%），遮罩只显示该范围：整幅方形位图作前景时四边 inset 必须为 **16.67%**，图案恰好铺满可视区完整显示；inset 25% 时图案只占可视区 75%（图标显小一圈并露出底色边框），inset 0% 时遮罩会裁掉原图四周各 1/6（图标显得被放大裁切）。启动页等直接绘制场景同理：自适应图标整体按 112dp 绘制只能得到 75dp 图案，需按 168dp 绘制（或取 `AdaptiveIconDrawable.foreground` 前景层按 168dp 绘制）才能得到 112dp 完整图案。
