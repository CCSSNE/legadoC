...
- `3.26.082210c` / `10707`，2026-08-22（UTC），`own` 主线（代码提交 `2f9c46f`，含 showBrowser 真实链路接入评论解析、preloadJs JS bridge 注入、融合诊断日志等最新已提交改动）。本次编译顺利：启动前确认工作区仅剩未跟踪文件、无外部活跃构建，`git stash -u` 移出后基于干净 HEAD 冷编译一次通过（BUILD SUCCESSFUL in 2m 40s，退出码 0）；构建后复查分支头未移动，stash 原样恢复。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082210c` / `10707`、中文名 `阅读 C`（zh locale）、架构 `arm64-v8a`；`apksigner` 完整执行退出码 0（仅 META-INF 未签名条目警告，可接受）。收尾 `gradlew --stop` 无守护进程、无残留构建 JVM。未安装到模拟器、未做正式回归（用户仅要求编译）。`origin/own` 为 `2f9c46f`，本地领先为基线记录提交。
