...
- `3.26.082407c` / `10727`，2026-08-24（UTC），`own` 主线（基于 `cc7f30a` 编译；版本号在先前交付 `10726` 之上单调递增）。用户明确要求编译时工作区干净（HEAD `cc7f30a`，含此前未交付的 `5070b1a`、`973d3cc`、`cc7f30a` 提交），本会话未改源码。按本机内存压力规则采用冷编译参数（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat` 隐藏窗口启动（`Start-Process -WindowStyle Hidden`），stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`。BUILD SUCCESSFUL in 4m 42s，退出码 0；构建期间分支无新提交（HEAD 保持 `cc7f30a`）。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082407c` / `10727`、中文名 `阅读 C`（zh locale）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（META-INF 未受签名保护条目提示可接受）。产物文件名原为 `legado_app_3.26.082407_10727.apk`，内部元数据正确，仅改名补 `c` 为 `legado_app_3.26.082407c_10727.apk`；旧 `10726` APK 已从产物目录移除。收尾 `gradlew --stop`，无残留 java/Gradle/kotlin 进程。基线记录提交正常推送；未安装到模拟器、未做正式回归。
