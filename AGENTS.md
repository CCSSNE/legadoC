...
- `3.26.083101c` / `10774`，2026-08-31（UTC 01 时），`own` 主线（编译锚点 `own @ 84e012e1`；基于基线 `10773`（锚点 `c4644419`）后新增提交 `84e012e1`（朗读引擎选择界面恢复原版斜角飘带标签：0f29f760曾将SYS/HTTP/V2徽标改为普通方块TextView且按主题色染色导致浅色主题白底白字不可读；现恢复BevelLabelView右上角45°飘带（36dp、飘带长24dp、accent红底白字），新增setText支持动态标签文本，showBadge去除error/primaryColor染色逻辑统一走飘带默认配色））。本次编译自动触发（持续监控到 `own` 分支出现新提交 `84e012e1` 后自动执行正式编译）。采用冷编译参数 `--no-daemon --max-workers=1` 全量编译成功，BUILD SUCCESSFUL in 2m 28s（75 actionable tasks: 20 executed, 55 up-to-date），err.log 为空，退出码 0。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.083101c` / `10774`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.083101_10774.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10773` APK 已删除。构建后 `gradlew --stop`、按 PID 清理残留 Gradle/Kotlin daemon；未安装到模拟器、未做正式回归。
