...
- 最近一次自用版交付为 `3.26.090919c` / `10892`，2026-09-09，基于提交 `1e355bac`（修复缓存管理评论加载中分支的进度条引用）使用 `assembleAppC` 编译成功（`BUILD SUCCESSFUL in 2m 30s`，`75 actionable tasks: 7 executed, 68 up-to-date`）。产物包名 `io.legado.app.dev`、versionName `3.26.090919c`、versionCode `10892`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner verify` 退出码 0；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090919_10892.apk`，尚未安装。
