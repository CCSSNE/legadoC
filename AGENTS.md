...
- 最近一次自用版交付为 `3.26.090609` / `10859`，2026-09-06，基于提交 `dae6b210`（`427f4e58` 修复Agent迁移悬空选择导致开局崩溃、`2b11c016` 贯彻破坏性升级：SCHEMA_VERSION升至3，对不上删了重建并删除旧迁移校验代码、`89b183cc` 修复fetch_source_html伪装成功并新增MCP自回归harness、`aafa54c5` 修复空历史悬空聊天指针、`e0a82ad2` 修复刷新监听按钮、`a7691c0c` 被动记忆跳过、`dae6b210` 版本门升3）使用 `assembleAppC` daemon 增量编译成功（默认参数，`BUILD SUCCESSFUL in 35s`，`75 actionable tasks: 13 executed, 62 up-to-date`，err 为空）。产物包名 `io.legado.app.dev`、versionName `3.26.090609c`、versionCode `10859`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner verify` 退出码 0（`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090609_10859.apk`（49978033 字节）。模拟器（emulator-5554）覆盖安装回归通过：自回归harness L1矩阵54/55（34工具全覆盖，唯一skip为联网调试）+L3假LLM聊天端到端全过（终稿/工具往返/落库）、冷启动无崩溃（新鲜v3）、AI设置7组入口正常、Server开关/握手/tools-list/tools-call/run落库全通后已关回停止、全程无FATAL。
