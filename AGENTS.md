...
- `3.26.081550c` / `10616`，2026-08-15，已在雷电模拟器安装。全局界面透明度统一为 `0` 不透明、`100` 全透明，换算由 `UiCorner.uiLayoutSurfaceAlpha()` 唯一负责；设置页截图实测 `100%` 时 UI 表面完全让出 Pre 背景。默认弹窗透明度为 `50`，默认弹窗模糊度为 `100`；阅读页透明背景默认开启。用户选定的 Pre 背景图已随 APK 内置，冷启动实测日间与夜间均安装到应用私有路径并与源图 SHA-256 一致。显式移除背景以空字符串保存，后续启动不会恢复默认图。
