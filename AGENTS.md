...
2. 局部模糊只允许作用于真实弹窗面板或专门的玻璃底层 View。Dialog 自己窗口内的外层根表面（包括填满底部 Dialog 窗口的读书面板）属于真实弹窗面板，不能误判成宿主整屏容器；找不到可靠目标时应放弃模糊，不能扩大到全局。
3. PixelCopy 的 `sourceRect` 必须使用源 Window 坐标：同窗用 `getLocationInWindow()` 相减；跨 Dialog/PopupWindow 时用屏幕坐标换算到宿主 decor，再加回宿主 decor 在源 Window 内的偏移，不能把屏幕坐标直接当 Window 坐标，并对矩形做严格相交裁剪，禁止用强制 1 像素裁剪掩盖坐标错误。
...
> 这是最近数小时对阅读主菜单、右上角“更多”菜单、书架菜单及普通弹窗反复实测后的止损规则。它不是说圆角在 Android 中永远不能去掉，而是**现有局部模糊结构下，直接去圆角已经反复失败并造成回归**；在有新的、可验证的结构方案前，必须保留当前圆角，不能继续空转试错。

1. **当前禁区范围**：阅读主菜单底部面板、阅读页“更多” PopupWindow、书架“更多”菜单，以及共用 Dialog/PopupWindow 的最外层玻璃背景。禁止直接把这些外壳的背景替换成直角矩形、透明空背景、纯白/纯灰色块，也禁止通过共用样式一次性移除圆角。
2. **已确认的冲突现象**：上述“直接去圆角”尝试会破坏承载局部模糊位图的背景层，表现为面板变白/变成实色、透明度失效、局部模糊只剩内部碎块或完全消失；随后即使把颜色改回去，也不能稳定恢复原有模糊效果。对此不能再以“补一个颜色/透明度兜底”继续覆盖。
3. **当前产品决策**：在完成独立外壳重构前，这些带局部模糊的面板**必须保留圆角**。任何“去圆角”需求都必须先标注为与当前模糊方案冲突，不能直接改代码后再靠编译包碰运气。
   - 当前可用基线是**最大 4dp 的小圆角**：仅通过弹窗/菜单专用的小圆角入口调整；禁止再次写 `0dp`、清空背景或改共用普通卡片圆角来影响弹窗。
4. **解除禁区的唯一前提**：先提出并验证全新的结构——直角几何外壳、承载模糊位图的背景层、内容层三者完全分离；每个实例使用独立背景副本；不能改动共用旧背景作为捷径。该结构须在模拟器上覆盖阅读主菜单、阅读“更多”、书架“更多”、普通 Dialog 四类界面，确认透明、模糊、点击、关闭/重开均稳定后，才允许再次讨论去圆角。
