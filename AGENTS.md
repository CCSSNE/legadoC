...
- `3.26.081703c` / `10637`，2026-08-16，已正式构建、APK 校验并安装到雷电模拟器，启动和 AI 设置页面无崩溃。章节净化请求根因已确认：DeepSeek V4 在 JSON 输出请求中需要 `response_format.type=json_object`，且默认 thinking 会使未约束的 JSON 流长时间无响应；结构化请求现统一发送 JSON Output 并关闭 thinking，普通聊天请求保持原有路径。响应状态和 Content-Type 已加入失败诊断；连通性测试提示词也明确要求 JSON。章节净化最终规则写入效果仍需在阅读页用真实操作完成端到端回归。
