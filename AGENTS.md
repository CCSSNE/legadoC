...
- `3.26.081658c` / `10635`，2026-08-16，已正式构建、APK 校验并安装到雷电模拟器。AI 设置新增当前模型真实连通性测试，未选择模型会直接提示；关闭复用时显示独立提供商、独立模型及其测试，重新开启则全部隐藏且配置保留。章节净化记录升级为成功/失败状态，失败不视为完成；移除了逐帧 `SELDBG`，避免淹没 AI 失败记录。已在雷电模拟器回归设置显隐、预检 Toast、Room 102 迁移启动和无崩溃。
