...
- `3.26.081735c` / `10659`，2026-08-17（GMT+8），已正式后台构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；已覆盖安装到雷电模拟器 `emulator-5554` 并完成实际回归。在 10658 基础上：修复无头弹窗底部操作行被内容区挤压的布局问题——`HeaderlessDialogChrome` 把 Toolbar 菜单动作搬到底部操作行，但老式布局里内容区是 `match_parent`（无 weight），会把追加在末尾的底部行挤成 0 高度导致按钮不可见；将 4 个受影响弹窗布局的内容区改为 `0dp + layout_weight="1"`：字体选择 `dialog_font_select.xml`（RecyclerView）、换封面 `dialog_change_cover.xml`（RecyclerView）、代码编辑 `dialog_code_view.xml`（CodeView）、HTTP-TTS 编辑 `dialog_http_tts_edit.xml`（NestedScrollView）。模拟器回归：字体弹窗底部「默认字体/其它目录」按钮可见可点（各弹出对应功能）、换封面弹窗底部「停止」按钮可见、HTTP-TTS 编辑弹窗底部「编辑内容/保存/登录/查看登录头」可见、输入框聚焦后点「编辑内容」正确打开 CodeEditActivity；无崩溃无异常日志。同模式排查结论：ConstraintLayout 宿主与 0dp+weight 内容区的其余弹窗（文件选择器、主题列表、TxtToc 规则、书组选择、验证码、WebDAV、变量、登录、换源等）由 chrome 正确锚定或本身留有底部行空间，不受此影响。上一交付 10658：手动编辑 / 反转内容无条件标记该章为已处理（COMPLETED、指纹=编辑后缓存原文 SHA-256、已处理类型并入编辑时启用的全部净化功能，自动 run 跳过且绝不为它生成替换规则；类型未覆盖时按常规判定重处理），回归覆盖无记录章自动处理、反转后记录转 COMPLETED、冷启动双章 SKIP_COMPLETED 零 AI 调用。版本号后续以此基线递增。
