...
- `3.26.081711c` / `10655`，2026-08-17（GMT+8），已正式后台构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；已覆盖安装到雷电模拟器 `emulator-5554` 并完成实际回归。AI 设置新增默认关闭的「高级设置」显示开关，关闭时保留供应商与模型常用项、日志和章节净化的去除广告/清理噪声/修正明显错别字三个开关，隐藏编辑请求、其余章节净化选项以及 AI 助手、MCP、联网工具和超时组；开启时恢复显示，所有配置值只隐藏不重置。AI 净化摘要的界面默认值和实际读取默认值均改为关闭，已有用户配置保持不变。既有 AI 设置分组和请求模板行为保持；针对性测试、正式构建、产物校验和模拟器回归通过；全量测试仍有既存无关失败 `ReadBookConfigTest.sanitize_clampsUnsafeLineSpacing`。
