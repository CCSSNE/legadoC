...
- `3.26.082909c` / `10760`，2026-08-29（UTC 9 时），`own` 主线（编译锚点 `own @ 736db6e`，`own` 与 `origin/own` 同步；基于基线 `10759` 后新增提交 `f9a3d4f`（TTS-Wav 模式默认关闭）、`dcdfb36`（页首按段默认关闭）、`1e73cf1`（百度离线TTS：朗读引擎入口 + 引擎管理界面）、`06e7c03`（百度离线TTS：多角色朗读简化版）、`505d9a4`（AI 创作各分组作用域默认值调整）。编译暴露源码错误，共 3 轮修复后同版本通过）。修复轮次：① `BdEngineManageActivity` 继承 `BaseActivity()` 缺少 ViewBinding 类型参数——`BaseActivity<VB : ViewBinding>` 要求提供类型参数与 binding 成员，原声明导致 `this` 无法解析为 Context，引发 `registerForActivityResult`/`primaryColor`/`toastOnUi`/`getPrefStringSet`/`contentResolver` 等一串级联错误；改为提供包装程序化根视图的 `ContentBinding`（`override val binding` 以 lazy 包裹 `buildContentView()`，onCreate 去掉重复 `setContentView`/`onActivityCreated` 由基类处理），并补 `primaryColor` 扩展导入——提交 `46560d7`；② `ContentBinding` 需显式实现 `ViewBinding` 的 `getRoot()` 方法：Kotlin 属性 `root` 未匹配 Java 接口 `getRoot()`，导致类型边界不满足引发 `toastOnUi` 级联错误，改为 `override fun getRoot(): View = contentView`——提交 `2227072`；③ 补 `toastOnUi`（`io.legado.app.utils` Context 扩展）导入——提交 `736db6e`。最终一轮：BUILD SUCCESSFUL in 2m 20s（75 actionable tasks: 7 executed, 68 up-to-date），err.log 为空，退出码 0；构建期间无新提交。失败包未产出、未交付。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082909c` / `10760`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.082909_10760.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10759` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`（仅限本构建 Gradle home）、无残留构建进程；未安装到模拟器、未做正式回归。
