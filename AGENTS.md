...
- `3.26.082423c` / `10738`，2026-08-24（UTC），`own` 主线（编译前锚点 `own @ 707b028`，工作区干净，`own` 与 `origin/own` 同步；与 10737 处于同一 UTC 小时，版本名时间部分同为 082423，升降级仅看 versionCode，10738 > 10737；基于基线 `10737` 后新增提交 `707b028 feat(bookshelf): mark shortcut books` 编译）。用户要求“编译一下”，按正式 `appC` 编译执行：核查无残留 Gradle/Kotlin/Java 进程、无 `hs_err_pid*`/`replay_pid*` 崩溃日志（内存余量 ~12.4GB），并按规则只删除产物目录中对应旧 APK `legado_app_3.26.082423c_10737.apk`；随后按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10738.bat`，字面量脚本回读校验 10 行后 `Start-Process -WindowStyle Hidden`，PID 11028）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`。BUILD SUCCESSFUL in 3m 10s（21 executed / 55 up-to-date），err.log 为空，退出码 0；构建期间无新提交，产物对应干净的 `707b028`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082423c` / `10738`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36、`application-debuggable`；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`（`legado_app_3.26.082423_10738.apk`），aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082423c_10738.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建产物 `app\schemas\...\106.json`（copyRoomSchemas 生成、未跟踪）按 569dc4a“Room 构建时重新生成”先例删除。构建后 `gradlew --stop`（无 daemon），确认无残留构建进程与崩溃日志；未安装到模拟器、未做正式回归。
