...
- `3.26.081621c` / `10626`，2026-08-15，已安装到雷电模拟器；按用户要求仅完成构建、APK 校验和覆盖安装，并重启应用触发误写偏好迁移，未额外进行功能回归。外部主题恢复为 `pre_default_background.png`；仅识别并修复上一版误写的日间 `护眼漫绿.jpg`、夜间 `宁静夜色.jpg` 私有路径。阅读页内置默认样式“微信读书”的日间/夜间背景仍由 `readConfig.json` 分别加载 `bg/护眼漫绿.jpg`、`bg/宁静夜色.jpg`。
