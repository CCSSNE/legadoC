...
- `3.26.082218c` / `10715`，2026-08-22（UTC），`own` 主线（基于 `8dc5919` 编译：refactor(cache) unify notifications and persist download logs + fix(log) AppLogDialog 缺失的 observeEvent import；版本号在先前产出 `10714` 之上单调递增）。本次编译分两轮：首轮按冷编译参数后台隐藏编译在 `compileAppCKotlin` 失败（BUILD FAILED，退出码 1，GRADLE_EXIT=1）——`AppLogDialog.kt:47` `Unresolved reference 'observeEvent'`，根因是提交 `dd32ff4` 新增的 `observeLiveBus()` 调用 `observeEvent`（`Fragment` 扩展）却漏掉 `import io.legado.app.utils.observeEvent`（库中全部其他调用点均有显式导入；事件载荷已确认与 `observeEvent<Int>` 匹配：`AppLog.postEvent(APP_LOG_CHANGED, mLogs.size / 0)`），属源码错误而非环境问题，未产出失败包；修复后同版本重编译一次通过（BUILD SUCCESSFUL in 4m 45s，退出码 0，同一冷编译参数）。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082218c` / `10715`、中文名 `阅读 C`（zh locale）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（仅 META-INF 未签名条目警告，可接受）。构建期间分支无新提交（HEAD 保持 `8dc5919`），无需重编。`values/strings.xml` 的 `qq_group_url` 多替换警告为既存非致命提示，与本次改动无关，未处理。收尾 `gradlew --stop` 无运行中守护进程，无残留 java 进程。未安装到模拟器、未做正式回归（用户仅要求编译）。推送后 `origin/own` 与本地 `own` 同步。
