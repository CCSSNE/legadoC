...
- `3.26.081706c` / `10650`，2026-08-17（GMT+8），已正式后台构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；已覆盖安装到雷电模拟器 `emulator-5554` 并完成实际回归。AI 设置的「章节净化」配置改为**引用全局供应商/模型，不再保存独立快照**：章节净化供应商从「重新填完整供应商表单」改为「从全局已有供应商中选择」（`AiChapterPurifyConfig.independentProviderId` 存供应商配置 id），章节净化模型从「手填模型 ID」改为「从所选供应商已全局添加的模型中选择」（`independentModelId` 存模型配置 id），运行时从 `AppConfig.aiProviderList` / `aiModelConfigList` 按 id 现查；切换所引用供应商时自动清空旧模型引用；旧版本 JSON 快照自动迁移取其 id；引用被全局删除时摘要显示「已失效（所选已被删除）」、执行时报错引导重选。全局「模型」分类下删除与「添加模型」重复的「获取模型列表」入口（`aiFetchModels`，线上拉取能力仍保留在「添加模型→从线上获取」）。「章节净化供应商/模型」标题改为「选择供应商/选择模型」，「测试独立模型连接」改为「测试所选模型连接」。真实回归覆盖：获取模型列表入口已删、选择供应商/选择模型均弹出纯选择列表（无表单/无手填）、摘要正确显示所引用名称、复用当前 AI 模型开关往返隐藏/恢复且引用保持、切换供应商正确清空模型、偏好存储确认以引用 id 落盘（非快照）。章节净化请求、日志、超时和实时状态能力保持上一基线行为。单元测试 `AiChapterPurifyHelperTest` 未受影响。
