...
- `3.26.082706c` / `10746`，2026-08-27（UTC），`own` 主线（编译前锚点 `own @ ace90a1`，`own` 与 `origin/own` 同步；基于基线 `10745` 后新增提交 `7397729`（朗读红字改绘制期投影）、`ace90a1`（修复 `invalidateReadAloudHighlight` 引用不存在的 `callBack.isScroll` 编译错误），UTC 为 08-27 06 时）。用户要求“编译”，按正式 `appC` 编译执行：首轮冷编译在 `compileAppCKotlin` 失败于上述 `isScroll` 引用错误；改用 `ReadView` 自身属性、提交 `ace90a1` 后同版本重编译。后台 `.bat`（`build_10746.bat`，字面量 here-string 回读校验 11 行后 `Start-Process -WindowStyle Hidden`，首启 PID 36772 / 重启 PID 22676）日志与退出码写入检出目录外 `D:\AI\audio\build-tmp`。重编译一次通过：BUILD SUCCESSFUL in 4m 20s（75 actionable tasks: 7 executed, 68 up-to-date），err.log 为空，退出码 0；构建期间无新提交。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082706c` / `10746`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`，按规则改名为 `legado_app_3.26.082706c_10746.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建产物 `app\schemas\...\106.json`/`107.json`（copyRoomSchemas 生成、未跟踪）按先例删除。构建后 `gradlew --stop`（无 daemon）；未安装到模拟器、未做正式回归。
