...
- 2026-09-04：后台 `Start-Process` 拉起 `gradlew.bat` 后，工具调用可能报 `ChildProcess.kill` 超时，但 Gradle 子进程实际仍在后台正常构建（以 out.log 是否增长与 java 进程 CPU 是否 climbing 为准，不得据此判定构建失败或重复拉起）。重复拉起会在同一检出目录形成多构建并发，必触发资源锁冲突与内存挤占；正确做法是用 `Get-CimInstance Win32_Process` 按命令行找回本次构建的 cmd/java PID，只保留唯一正确参数构建并 `Wait-Process` 等待，其余本次杂散构建按 PID 清理（不得碰非本次进程）。
### 双构建路线（自有 / 开源）
...
- 最近一次自用版交付为 `3.26.090323` / `10825`，2026-09-04，基于提交 `bac9ae80`（聚合主页内容容器统一加顶部边距，卡片横幅行根顶部边距收回容器处避免叠加）使用 `assembleAppC` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，`BUILD SUCCESSFUL in 4m 23s`；拉起时工具误报 `ChildProcess.kill` 但后台构建正常，经 `Get-CimInstance` 确认唯一构建后 `Wait-Process` 等待）。产物包名 `io.legado.app.dev`、versionName `3.26.090323c`、versionCode `10825`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过（退出码 0，`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090323_10825.apk`。注：versionName 与上一自用版同处 `090323` 小时窗口（UTC 编译时间），versionCode 递增保证升级。上一自用版为 `3.26.090323` / `10824`；最近公开版为 `3.26.090318` / `10819`，2026-09-03，使用 `assembleOssRelease` 冷编译成功（`-Pkotlin.compiler.execution.strategy=in-process`）。产物包名 `io.legado.app.c`、versionName `3.26.090318`、versionCode `10819`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\oss\release\legado_oss_3.26.090318_10819.apk`。
