...
- `3.26.082403c` / `10725`，2026-08-24（UTC），`own` 主线（基于 `1d0bcc4` 编译；版本号在先前交付 `10724` 之上单调递增）。用户明确要求正式编译另一会话的新提交（`6a2704a`、`5929c0c` review 并发重构）；工作区另有该会话刚创建的未跟踪目录 `docs/流程图/`（含空占位文件），经用户确认由本会话代为提交（`4e488b9`）后编译。首次编译在 `compileAppCKotlin` 失败：`ReviewSnapshotCapture.kt` 新代码使用 `File`/`AppConfig` 未加 import（7 处报错，其中 `size` 为类型未解析的级联错误），补 `import java.io.File` 与 `import io.legado.app.help.config.AppConfig` 后提交 `1d0bcc4`，以同一 versionCode/versionName 重编译成功（失败包未产出、未交付）：BUILD SUCCESSFUL in 1m 24s，退出码 0。构建期间分支无新提交（HEAD 保持 `1d0bcc4`）。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082403c` / `10725`、中文名 `阅读 C`（zh locale）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行（不截断管道）退出码 0。产物文件名原为 `legado_app_3.26.082403_10725.apk`，内部元数据正确，仅改名补 `c` 为 `legado_app_3.26.082403c_10725.apk`。收尾 `gradlew --stop`，无残留 java 进程。基线记录提交正常推送；未安装到模拟器、未做正式回归。
