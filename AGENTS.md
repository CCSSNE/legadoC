...
- `3.26.081737c` / `10661`，2026-08-17（GMT+8），`feature/audio-reading-ui-unification` 分支，已正式后台构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；已覆盖安装到雷电模拟器 `emulator-5554` 并完成启动回归。本次为音频书融入普通阅读的第一阶段功能集合：`BookTocLoadingActivity` 中音频书不再跳转 `AudioPlayActivity` 而是进入 `ReadBookActivity`；新增统一引擎接口 `ReadAloudEngine` 及 `ReadAloudEngineManager`、`UnifiedReadAloudService`，书源音频走 `SourceAudioEngine`（ExoPlayer）、HTTP-TTS 走 `HttpTtsEngine`、系统 TTS 走 `SystemTtsEngine`；`ContentProcessor` 对音频书优先显示歌词/字幕（`chapter.variable.lyric`），无歌词时显示占位文本；`dialog_read_aloud.xml` 新增进度条面板，`ReadAloudDialog` 接入初始化与事件。编译过程中修复一个编译错误：`IntentAction.adjustSpeed` 改为实际常量 `IntentAction.setSpeed`（提交 `373b495`）。模拟器回归：覆盖安装成功（10660 → 10661）、`MainActivity` 正常显示书架/底部导航/搜索按钮，无 FATAL 崩溃，logcat 仅书架空封面 Glide null 提示与模拟器 fs-verity 不支持警告。上一交付 10659：修复无头弹窗底部操作行被内容区挤压的布局问题——`HeaderlessDialogChrome` 把 Toolbar 菜单动作搬到底部操作行，但老式布局里内容区是 `match_parent`（无 weight），会把追加在末尾的底部行挤成 0 高度导致按钮不可见；将 4 个受影响弹窗布局的内容区改为 `0dp + layout_weight="1"`：字体选择 `dialog_font_select.xml`（RecyclerView）、换封面 `dialog_change_cover.xml`（RecyclerView）、代码编辑 `dialog_code_view.xml`（CodeView）、HTTP-TTS 编辑 `dialog_http_tts_edit.xml`（NestedScrollView）。版本号后续以此基线递增。
