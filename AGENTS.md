...
- 标准循环是：**自主复现问题 → 采集证据并定位根因 → 修改源码 → 按 Git 规则提交、推送 → 自动正式编译 `appC` → 安装到 serial / device 标识以 `emulator` 开头的雷电模拟器 → 自动运行和测试/回归 → 分析结果 → 若仍有问题则继续定位、改代码、提交、再编译、再安装、再测试**，持续迭代直到问题被验证解决。
...
- 用户明确要求“完整验证闭环”时，执行：正式编译 `appC` APK → 安装到 serial / device 标识以 `emulator` 开头的雷电模拟器 → 复现并回归验证。
...
- **模拟器身份只认 ADB serial / device 标识的 `emulator` 前缀。** 例如 `emulator-5554`：因为它**以 `emulator` 开头**，所以它就是允许操作的模拟器。
- **只有以 `emulator` 开头的目标才是模拟器。没有这个前缀的，一律不是模拟器。** 这是一条绝对判定，不存在第二套识别方法。
- **不得对模拟器身份进行任何额外探究、推断、验证或“二次确认”。** 不查设备型号、品牌、厂商、product、model、transport、USB/TCP 来源、端口规律、系统属性、`getprop`、LDPlayer 实例信息或任何其他身份线索；这些信息都不得用于改变判定结果。
- **不要判断“它看起来像不像模拟器”，只看 `emulator` 这个单词是否位于 serial / device 标识开头。** `emulator-*` = 模拟器；其他任何形式 = 不是模拟器，绝对禁止对其执行 ADB、截图、点击、安装、卸载、push/pull、shell、uiautomator2、Frida 或其他设备操作。
- 所有 ADB / uiautomator2 / Frida 等设备侧工具在执行前，只做这一项机械检查：目标标识是否以 `emulator` 开头。**是则允许按当前工作模式继续；不是则立即停止。不得继续调查该目标到底是什么设备。**
- **再次强调：模拟器身份认定只认 `emulator` 前缀。认准这个单词，不要想任何其他东西。**
- 真实小说优先用于阅读功能验证。`C:\Users\user\Documents\leidian14\Pictures` 与模拟器 Pictures 目录互通，可作为导入素材。

### 验证闭环

本节在两种情况下适用：**用户明确要求对应正式验证动作，或用户明确启用全自动模式。** 非全自动模式下，用户明确要求完整验证闭环时，按以下流程执行：正式编译 `appC` APK -> 安装到 serial / device 标识以 `emulator` 开头的雷电模拟器 -> 复现并回归验证；用户只要求其中一项时，只执行该项，不自行补充其余步骤。**全自动模式下则由 AI 自主重复该闭环，并根据每轮结果继续修改和迭代，无需用户逐项再次授权。**

- 模拟器不可用时不得改用真机。
- 崩溃或行为异常时，先按当前模式完成原因定位。**非全自动模式**只有用户已明确要求正式验证闭环时，才在正式修复后继续正式编译和回归；**全自动模式**则自动进入编译、安装、测试和回归，并在失败时继续下一轮修改。不能报告未经对应模式验证的修复。
