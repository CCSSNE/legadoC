...
- `3.26.082323c` / `10719`，2026-08-23（UTC），`own` 主线（基于 `55fd96b` 编译：fix(cache): start body units on execution）。冷编译一次通过（BUILD SUCCESSFUL in 2m 7s，退出码 0）。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082323c` / `10719`、中文名 `阅读 C`（zh locale）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（仅 META-INF 未签名条目警告，可接受）。构建期间分支无新提交（HEAD 保持 `55fd96b`），无需重编；`gradlew --stop` 确认无运行中的守护进程。用户只授权正式编译，未安装到模拟器、未做正式回归。
