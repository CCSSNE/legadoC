...
- `3.26.081701c` / `10645`，2026-08-17（GMT+8），已正式构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；本次雷电模拟器 `emulator-5554` 离线，未安装和回归。章节净化先通过 `ContentProcessor` 应用当前启用的普通净化替换规则，再把处理后的文本送给 AI；AI 新规则继续追加到现有最大排序之后，完成记录使用实际 AI 输入指纹，并记录原文与处理后文本诊断。AI 流式请求已区分 SSE 空闲超时和生成总超时，支持实时思考/输出 token 与 TPS 进度；阅读页实时状态 Snackbar 在任务结束前持续显示，只在成功完成、失败、取消、超时或切换排队任务时收起。
