...
- `3.26.082705c` / `10744`，2026-08-27（UTC），`own` 主线（编译前锚点 `own @ 9b1838b`，`own` 与 `origin/own` 同步；基于基线 `10743` 后新增提交 `c1d4254`（抽取统一朗读高亮对账并增加页面显示驱动入口）、`81e9000`（朗读位置确认不再解除页面脱钩）、`9b1838b`（朗读高亮只对账当前显示页、不再预写离屏页），UTC 已进入 08-27 05 时，版本名为 `3.26.082705`，`versionCode` 按 10744 > 10743 递增。工作区仅两个未跟踪 `docs/org/*.png` 截图，不参与 APK 打包）。用户要求“编译”，按正式 `appC` 编译执行：核查无残留 Gradle/Kotlin/Java 进程；随后按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10744.bat`，字面量 here-string 回读校验 9 行后 `Start-Process -WindowStyle Hidden`，PID 4704）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`。编译一次通过：BUILD SUCCESSFUL in 1m 59s（76 actionable tasks: 14 executed, 62 up-to-date），err.log 为空，退出码 0；构建期间无新提交，产物对应干净的 `9b1838b`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082705c` / `10744`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`，aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082705c_10744.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建产物 `app\schemas\...\106.json`/`107.json`（copyRoomSchemas 生成、未跟踪，本轮无数据库版本变更）按先例删除。构建后 `gradlew --stop`（无 daemon），确认无残留构建进程；未安装到模拟器、未做正式回归。
