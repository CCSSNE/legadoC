...
- `3.26.082416c` / `10735`，2026-08-24（UTC），`own` 主线（基于 `75826e5` 编译；编译前锚点 `own @ 75826e5`，工作区干净，`own` 与 `origin/own` 同步；版本号高于最近有效交付 `10734`）。用户要求“编译，编译”，按正式 `appC` 编译执行：核查无残留 Gradle/Kotlin/Java 进程、无 `hs_err_pid*`/`replay_pid*` 崩溃日志（内存余量 ~8.8GB），并按规则只删除产物目录中对应旧 APK `legado_app_3.26.082416c_10734.apk`；随后按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10735.bat`，`Start-Process -WindowStyle Hidden`，PID 40704）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`。BUILD SUCCESSFUL in 3m 22s，退出码 0（13 executed / 62 up-to-date），err.log 为空；构建期间无新提交，产物对应干净的 `75826e5`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082416c` / `10735`、中文名 `阅读 C`（zh/zh-CN/zh-HK/zh-TW）、架构 `arm64-v8a`、compileSdk/targetSdk 36、`application-debuggable` 与 `10734` 基线一致；`apksigner` 完整执行退出码 0（META-INF 未受签名保护条目提示可接受）。产物原始文件名缺 `c`（`legado_app_3.26.082416_10735.apk`），aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082416c_10735.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建后 `gradlew --stop`，确认无残留进程与崩溃日志；未安装到模拟器、未做正式回归。
