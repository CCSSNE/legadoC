...
- `3.26.081709c` / `10653`，2026-08-17（GMT+8），已正式后台构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；已覆盖安装到雷电模拟器 `emulator-5554` 并完成实际回归。本次改动：AI 章节净化流程说明弹窗文案更新为「A 书源原文 → B 没开AI净化的状态 → C 送入AI大模型的文本 → D 净化后的成品」四阶段 + 预处理作用域（ad：广告识别 / typo：修正错别字 / noise：清理噪声），摘要同步改为「查看 A、B、C、D 四阶段文本处理流程和预处理作用域」。真实回归覆盖：弹窗打开文案逐字正确、确定关闭、应用重启后再次打开内容稳定、全程无崩溃。此前已交付并保持：AI 设置七组（「供应商与模型」「AI 章节净化」「日志」「AI 助手」「AI MCP 服务器」「AI 联网工具」「超时」）按表格顺序排列，`aiAssistantEnabled`、`aiChapterPurifyTestConnection`、`aiSseIdleTimeoutSeconds` 分别命名为「AI 助手启用」「测试独立模型连接」「首字延迟超时」；章节净化关闭「复用当前 AI 模型」时显示选择供应商、选择模型、独立编辑请求和测试独立模型连接，开启时四项全部隐藏；独立请求模板只在用户保存时写入，未配置时继承全局模板，编辑器支持恢复全局请求。
