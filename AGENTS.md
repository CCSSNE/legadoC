...
- 最近一次自用版交付为 `3.26.090323` / `10824`，2026-09-04，基于提交 `2d0ad800`（聚合主页取消单选多选区分：统一选择分类入口与tab头）使用 `assembleAppC` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，`BUILD SUCCESSFUL in 5m 12s`；拉起时工具误报 `ChildProcess.kill` 但后台构建正常，清理并发重复构建后仅保留唯一构建）。产物包名 `io.legado.app.dev`、versionName `3.26.090323c`、versionCode `10824`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090323_10824.apk`。上一自用版为 `3.26.090322` / `10823`；最近公开版为 `3.26.090318` / `10819`，2026-09-03，使用 `assembleOssRelease` 冷编译成功（`-Pkotlin.compiler.execution.strategy=in-process`）。产物包名 `io.legado.app.c`、versionName `3.26.090318`、versionCode `10819`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\oss\release\legado_oss_3.26.090318_10819.apk`。
