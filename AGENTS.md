...
- `3.26.090105` / `10793`，2026-09-01（UTC 05 时），`own` 主线，**自用版 appC 单编译交付**（编译锚点 `own @ 9836b465`；基于基线 `10792` 后新增提交 `9836b465`（高亮规则预置阅读 Max 内置规则，默认全部关闭））。本次为用户点名"编译自用版本"，确认按新版本交付（versionCode 递增 10793）。`-Pabi=arm64-v8a -PVERSION_CODE=10793 -PVERSION_NAME=3.26.090105`，冷编译 `--no-daemon --max-workers=1 -Dorg.gradle.parallel=false -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`。`assembleAppC` BUILD SUCCESSFUL in 2m 7s，`aapt` 确认包名 `io.legado.app.dev`、versionName `3.26.090105c`、versionCode `10793`、arm64-v8a、应用名 `阅读 C`；产物 `app\build\outputs\apk\app\c\legado_app_3.26.090105_10793.apk`（48.39 MB），备份于 `D:\AI\audio\build-tmp\legado_app_3.26.090105_10793.apk`。`apksigner` 退出码 0，证书 `CN=Android Debug`、SHA-256 `70cb88ca...`（debug 签名，符合只允许 release 系构建 + 统一 debug 签名的签名铁律）。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。沿用此前批次设定的 `GRADLE_USER_HOME=D:\AI\audio\android-gradle-user-home`（与项目同盘），未再触发 KSP 跨盘 bug。
