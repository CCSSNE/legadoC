...
- `3.26.082702c` / `10741`，2026-08-27（UTC），`own` 主线（编译前锚点 `own @ 8922c52`，`own` 与 `origin/own` 同步；基于基线 `10740` 后新增提交 `d2bd336`（重构朗读位置与页面跟随流程）、`9d6db85`（收口朗读跟随与进度数据源）、`fb15f65`（修复脱钩朗读高亮重绘）、`be85ea2`（修复朗读跨页高亮残留）、`8922c52`（完善脱钩朗读跨章节高亮清理）编译，UTC 已进入 08-27 02 时，版本名为 `3.26.082702`，10741 > 10740。工作区仅两个未跟踪 `docs/org/*.png` 截图，不参与 APK 打包）。用户要求“编译”，按正式 `appC` 编译执行：核查无残留 Gradle/Kotlin/Java 进程、无崩溃日志，并按规则只删除产物目录中对应旧 APK `legado_app_3.26.082623c_10740.apk`（`build-tmp` 已有备份）；随后按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10741.bat`，字面量脚本回读校验 9 行后 `Start-Process -WindowStyle Hidden`，PID 20344）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`。编译一次通过：BUILD SUCCESSFUL in 3m 20s（76 actionable tasks: 14 executed, 62 up-to-date），err.log 为空，退出码 0；构建期间无新提交，产物对应干净的 `8922c52`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082702c` / `10741`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`，aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082702c_10741.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建产物 `app\schemas\...\106.json`/`107.json`（copyRoomSchemas 生成、未跟踪，本轮无数据库版本变更）按先例删除。构建后 `gradlew --stop`（无 daemon），确认无残留构建进程与崩溃日志；未安装到模拟器、未做正式回归。
