...
- `3.26.081713c` / `10657`，2026-08-17（GMT+8），已正式后台构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；已覆盖安装到雷电模拟器 `emulator-5554` 并完成实际回归。AI 章节净化引入「章节处理记录」状态机：`ai_chapter_purify_records` 新增 `processedTypes` 列（DB 102→103 迁移，旧记录保留并在升级后一次性全量重扫）；章节指纹改为**仅对章节缓存原文算 SHA-256**（与规则/设置无关），处理器记录该章实际被哪些 AI 功能处理过（`processedTypes`）；跳过判定 = 记录存在且指纹匹配且已处理类型覆盖当前启用类型；force（刷新/重新下载/换源）只作用于批量窗口的起始章，邻章仍受跳过门控（修复第 1 章刷新连带重处理第 2 章的问题）；编辑内容/反转内容只更新记录指纹不触发 AI（`markChapterEdited`）；清除缓存/更新目录/移除书架删除该书记录（`dropBookRecords`），重读时全量重算。模拟器回归：skip 门控（force=false 双章 SKIP_COMPLETED、零 AI 调用）、缓存原文变化自动重处理、force 刷新后邻章记录保持不变（DB 级证明连坐修复）、force 后写入指纹与真实缓存 SHA-256 一致，均验证通过。同版本还包含：AI 思考打断默认值改为五秒、AI 报错信息统一为中文第一句分类（仅文案，判定逻辑不变）。版本号后续以此基线递增。
