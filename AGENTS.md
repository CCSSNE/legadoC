...
- 最近一次自用版交付为 `3.26.090322` / `10821`，2026-09-03，基于最新提交 `63e30b35`（修正欢迎页 logo 尺寸：Android 12+ 启动屏按 108dp 画布管线缩放）使用 `assembleAppC` 冷编译一次成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，`BUILD SUCCESSFUL in 4m 43s`）。产物包名 `io.legado.app.dev`、versionName `3.26.090322c`、versionCode `10821`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090322_10821.apk`。上一自用版为 `3.26.090320` / `10820`；最近公开版为 `3.26.090318` / `10819`，2026-09-03，使用 `assembleOssRelease` 冷编译成功（`-Pkotlin.compiler.execution.strategy=in-process`）。产物包名 `io.legado.app.c`、versionName `3.26.090318`、versionCode `10819`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\oss\release\legado_oss_3.26.090318_10819.apk`。
