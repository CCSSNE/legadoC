...
- `3.26.090103` / `10791`，2026-09-01（UTC 03 时），`own` 主线（编译锚点 `own @ 8e4e7091`；基于基线 `10790`（锚点 `aafb1338`）后新增提交 `cdff666c`（文档：朗读逻辑再升级，去掉硬门控，提升性能，研发笔记）、`15b4d5b2`（修复有声书返回阅读页时朗读目标章越界导致的崩溃，失败降级为保持当前章并打日志暴露）、`486359c6`（正文长按条新增搜索功能：内嵌 WebView 搜索面板与自定义搜索引擎管理）、`655acbc5`（新增高亮规则：正则命中的正文文字应用显示效果）、`8e4e7091`（修复背景图路径失效导致书籍详情页崩溃：三个背景读取口收敛到统一解析入口））。**本次为自用版+公开版双编译交付**（用户先后要求双编译、随即改为只编自用版、随后又要求补编公开版，实际两版均完成）。`-Pabi=arm64-v8a -PVERSION_CODE=10791 -PVERSION_NAME=3.26.090103`，冷编译 `--no-daemon --max-workers=1 -Dorg.gradle.parallel=false -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`。**自用版 `assembleAppC`**（增量 23s）产物 `app\build\outputs\apk\app\c\legado_app_3.26.090103_10791.apk`（48.39 MB），包名 `io.legado.app.dev`、versionName `3.26.090103c`、应用名 `阅读 C`。**公开版 `assembleOssRelease`**（BUILD SUCCESSFUL in 7m 41s）产物 `app\build\outputs\apk\oss\release\legado_oss_3.26.090103_10791.apk`（34.74 MB），包名 `io.legado.app.c`、versionName `3.26.090103`（无 c 后缀）、应用名 `阅读`。两者均已备份于 `D:\AI\audio\build-tmp\`，`apksigner` 退出码均 0，证书 `CN=Android Debug`、SHA-256 `70cb88ca...`（debug 签名，符合只允许 release 系构建 + 统一 debug 签名的签名铁律）。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。本批次曾两次因 KSP 跨盘 bug（gradle 缓存误落 C 盘 vs 项目 D 盘，`this and base files have different roots`，glide-svg 注解处理器触发）在 KSP 任务失败；后台 `.bat` 内联设置 `GRADLE_USER_HOME=D:\AI\audio\android-gradle-user-home`（与项目同盘）后 KSP 正常，同版本重编译成功，失败包未产出。
