...
- 2026-09-03：自适应图标（`mipmap-anydpi-v26`）可视区仅为 108dp 画布中央的 72dp（66.7%），遮罩只显示该范围：整幅方形位图作前景时四边 inset 必须为 **16.67%**，图案恰好铺满可视区完整显示；inset 25% 时图案只占可视区 75%（图标显小一圈并露出底色边框），inset 0% 时遮罩会裁掉原图四周各 1/6（图标显得被放大裁切）。启动页等直接绘制场景同理：自适应图标整体按 112dp 绘制只能得到 75dp 图案，需按 168dp 绘制（或取 `AdaptiveIconDrawable.foreground` 前景层按 168dp 绘制）才能得到 112dp 完整图案。最终采用的做法：瓷贴位图本身按"图案占画布 2/3 + 外圈透明"制作（以 docs 里 ICO 的 256px 成品帧为唯一来源，安卓不识别 .ico，须提取帧转 PNG），`foreground` 直接引用瓷贴、不再用 inset 包装层，主题启动页同帧按 168dp 绘制。
- 2026-09-03：欢迎页（冷启动屏）默认路径绝不能替换窗口背景。冷启动的系统启动窗口按**系统**夜间模式解析主题 `windowBackground`（`bg_welcome_preview`），而应用内日夜是 AppCompat 运行时切换，两者不一致时（应用内夜间+系统日间等）任何在欢迎页再叠加的"应用内解析底色"都会造成白↔黑闪屏；老版本默认不动背景所以日夜都正常。`Context.backgroundColor` 扩展（MaterialValueHelper.kt）在配置了背景图时返回 `Color.TRANSPARENT`，只能用于已有内容衬底的场景，画到空窗口上会露出黑色窗口（曾导致默认启动页全黑）。恢复的语义：`upBackgroundImage()` 仅在设置了自定义启动图（受 `customWelcome` 开关与文件存在校验）时替换背景，默认一律保持主题 `bg_welcome_preview`（固定底色+默认图标瓷贴）；启动页 logo 因此只能静态跟随默认图标，无法跟随 Launcher 别名切换，这是日夜一致性的必要取舍。
...
- 最近一次自用版交付为 `3.26.090308` / `10817`，2026-09-03，基于最新提交 `27cf4489`（恢复老版本欢迎页语义）使用 `assembleAppC` 冷编译成功（`-Pkotlin.compiler.execution.strategy=in-process`）。产物包名 `io.legado.app.dev`、versionName `3.26.090308c`、versionCode `10817`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090308_10817.apk`。本次未安装模拟器、未做正式回归。上一自用版为 `3.26.090307` / `10816`；最近公开版仍为 `3.26.090209` / `10804`。
