...
- `3.26.082406c` / `10726`，2026-08-24（UTC），`own` 主线（基于 `6accff1` 编译；版本号在先前交付 `10725` 之上单调递增）。首次尝试 10726 / `3.26.082405`（UTC）在 `compileAppCKotlin` 失败：`ReviewSnapshotStatusDialog.kt` 对 `recyclerView`/`btnRetryAll`/`rotateLoading`/`tvEmpty`/`setText` 报 Unresolved reference（失败包未产出、未交付）；错误已在后续提交中修复，本会话未再改源码（当前 HEAD 布局 ID 蛇形 `recycler_view`/`rotate_loading`/`btn_retry_all`/`tv_empty` 与 ViewBinding 驼峰字段对应无误）。用户删除了失败尝试遗留的构建日志/脚本，并要求代为提交未跟踪的 docs 改动（`4e488b9` 风格），提交 `6accff1`（docs(flow): clarify currentChapterIndex and auto chapter switching）后重新编译。编译前清理旧失败日志；构建期间分支无新提交（HEAD 保持 `6accff1`）：BUILD SUCCESSFUL in 5m 53s，退出码 0（CompileAppCKotlin 仅 deprecation 警告，无错误）。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082406c` / `10726`、中文名 `阅读 C`（zh locale）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行（不截断管道）退出码 0。产物文件名原为 `legado_app_3.26.082406_10726.apk`，内部元数据正确，仅改名补 `c` 为 `legado_app_3.26.082406c_10726.apk`；旧 `10725` APK 已从产物目录移除。收尾 `gradlew --stop` 并按 PID 清理残留 Gradle/Kotlin daemon，无残留 java 进程。基线记录提交正常推送；未安装到模拟器、未做正式回归。
