...
- 最近一次自用版交付为 `3.26.090322` / `10823`，2026-09-03，基于最新提交 `94fd8a89`（回退 `b00bd65e` 恢复欢迎页 logo 半尺寸并加保护注释：v31 54dp 与默认 96dp 为 AOSP 管线公式值）使用 `assembleAppC` 冷编译一次成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，`BUILD SUCCESSFUL in 4m 44s`）。产物包名 `io.legado.app.dev`、versionName `3.26.090322c`、versionCode `10823`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090322_10823.apk`。上一自用版为 `3.26.090322` / `10822`；最近公开版为 `3.26.090318` / `10819`，2026-09-03，使用 `assembleOssRelease` 冷编译成功（`-Pkotlin.compiler.execution.strategy=in-process`）。产物包名 `io.legado.app.c`、versionName `3.26.090318`、versionCode `10819`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\oss\release\legado_oss_3.26.090318_10819.apk`。
