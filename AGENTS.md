...
- `3.26.082815c` / `10754`，2026-08-28（UTC），`own` 主线（编译前锚点 `own @ 30bc7eb`，`own` 与 `origin/own` 同步；基于基线 `10753` 后新增提交 `bcf6700`（朗读新增 TTS-Wav 模式，默认开）、`30bc7eb`（收录朗读模块第3次重构核心流程图）。用户要求“编译”，按正式 `appC` 编译执行：工作区 22 个 stat 缓存脏标记文件经 hash-object 与 HEAD 逐一比对确认内容完全一致，`git add` 刷新索引后干净；按内存压力规则冷编译 `:app:assembleAppC`。后台 `.bat`（`build_10754.bat`，回读校验后 `Start-Process -WindowStyle Hidden`）日志与退出码写入 `D:\AI\audio\build-tmp`。编译一次通过：BUILD SUCCESSFUL in 3m 52s（75 actionable tasks: 20 executed, 55 up-to-date），err.log 为空，退出码 0；构建期间无新提交。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082815c` / `10754`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物文件名已含版本 `legado_app_3.26.082815_10754.apk`（Gradle 自动命名，无需改名），收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10753` APK 已删除。构建后 `gradlew --stop`（无 daemon）、无残留构建进程；未安装到模拟器、未做正式回归。
