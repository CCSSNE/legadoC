...
- 最近一次自用版交付为 `3.26.090916c` / `10888`，2026-09-09，基于提交 `ac791571`（按实际绘制位置动态锚定离线评论栏）使用 `assembleAppC` 编译成功（`BUILD SUCCESSFUL in 32s`，`75 actionable tasks: 13 executed, 62 up-to-date`）。产物包名 `io.legado.app.dev`、versionName `3.26.090916c`、versionCode `10888`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner verify` 退出码 0；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090916_10888.apk`，尚未安装。
