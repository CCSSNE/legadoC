...
- `3.26.082820c` / `10755`，2026-08-28（UTC 20 时），`own` 主线（编译前锚点 `own @ a2977de`，`own` 与 `origin/own` 同步；基于基线 `10754` 后新增 TTS 缓存系列提交 `bda1fb9`/`c8e292b`/`7fbc222`/`92cef2c`/`4b7eab3`，以及编译错误修复提交 `a2977de`）。用户要求“编译”，按正式 `appC` 编译执行：首次冷编译在 `:app:compileAppCKotlin` 失败，11 个 Kotlin 错误集中于 TTS 缓存 4 个文件（缺 `isAudio`/`isVideo`/`simulatedTotalChapterNum`/`fromJsonObject`/`setLayout`/`collect` 导入、`DEFAULT_ENGINE_KEY`/`DEFAULT_VOICE_KEY` 被误标 `private`、`QueueTask.executionLease` 错误可空），逐项根因修复后提交 `a2977de` 并推送，同版本重编。失败包未产出、未交付。重编一次通过：BUILD SUCCESSFUL in 5m（75 actionable tasks: 9 executed, 66 up-to-date），err.log 为空，退出码 0；构建期间另一会话 Gradle daemon 构建的是 `D:\AI\audio\android`（非同一工作区，无 R.jar 锁冲突）。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082820c` / `10755`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.082820_10755.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10754` APK 已不在输出目录。构建后 `gradlew --stop`（仅限本构建 Gradle home，未触碰另一会话 daemon）、无残留构建进程；未安装到模拟器、未做正式回归。
