...
别的 Agent 都护着用户数据，我们反着来：AI 相关全部配置一律破坏性升级，今后写 AI 一律沿此开关。版本检测用现成的 `AgentConfig.SCHEMA_VERSION`，改结构就加一，对不上即过期；过期只删旧建新——不校验、不迁移、不兼容，按随包默认与内置种子重建。按零用户设计，不为不存在的用户与数据做任何保护。
