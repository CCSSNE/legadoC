...
- `3.26.082704c` / `10743`，2026-08-27（UTC），`own` 主线（编译前锚点 `own @ 5e39200`，`own` 与 `origin/own` 同步；基于基线 `10741` 后新增提交 `8931045`（防止朗读位置事件乱序污染界面）、`9c8af54`（修复页首按段失效并与按页朗读假联动）、`5e39200`（修正按页模式起点偏移为页片段内偏移），UTC 已进入 08-27 04 时，版本名为 `3.26.082704`；产物目录发现一个未记录的同日 APK `legado_app_3.26.082703_10742.apk`（内部 `3.26.082703c`/`10742`），无法确认其包含最新 HEAD 提交集合，故不作为交付物，移入 `D:\AI\audio\build-tmp` 备份后按新 HEAD 重编译并递增为 `10743` > 10742。工作区仅两个未跟踪 `docs/org/*.png` 截图，不参与 APK 打包）。用户要求“编译”，按正式 `appC` 编译执行：核查无残留 Gradle/Kotlin/Java 进程；随后按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10743.bat`，字面量脚本回读校验 11 行后 `Start-Process -WindowStyle Hidden`，PID 23536）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`。编译一次通过：BUILD SUCCESSFUL in 2m 9s（76 actionable tasks: 14 executed, 62 up-to-date），err.log 为空，退出码 0；构建期间无新提交，产物对应干净的 `5e39200`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082704c` / `10743`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`，aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082704c_10743.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建产物 `app\schemas\...\106.json`/`107.json`（copyRoomSchemas 生成、未跟踪，本轮无数据库版本变更）按先例删除。构建后 `gradlew --stop`（无 daemon），确认无残留构建进程；未安装到模拟器、未做正式回归。
