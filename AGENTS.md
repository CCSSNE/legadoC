...
- `3.26.081607c` / `10619`，2026-08-15，已在雷电模拟器安装。书架封面透明度默认 `30%`，`0%` 不透明、`100%` 全透明，只控制书架普通书封面与书架合集拼图；“加入合集”弹窗中的合集封面继续使用弹窗表面透明度，不受此项影响。原“书页背景透明”布尔开关及其偏好键、备份项、阅读页分支已移除，阅读页背景只使用自身背景透明度。真实路径“我的 → 界面设置”已确认旧开关不显示，新百分比项显示 `30%`；弹窗透明度 `50%`、全局界面透明度 `0%` 保持独立。
