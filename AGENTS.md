...
- `3.26.083012c` / `10768`，2026-08-30（UTC 12 时），`own` 主线（编译锚点 `own @ 900ce9e5`；基于基线 `10767` 后新增提交 `f97abc67`（对齐NG引擎体系第四步：移植合成客户端与引擎存储，脚本客户端HTTP/WS/SSE、本机转发器、流式音频、TtsEngineStore导入导出与发音人目录缓存，ReadAloud引擎V2集成面）、`7c0c719b`（第五步：选角绑定泛化为（引擎,音色）并移植ReadAloudTtsRouter，book_tts_voice_bindings加engineId列111→112迁移，内置语音包引擎外观统一路由表达，全局兜底兼容旧偏好）、`86812c83`（第六步：朗读管线接入V2引擎，HttpReadAloudService脚本引擎合成路径与refreshTtsRoute热刷新，ReadAloud路由派生V2接管逻辑，百度宿主选角走ReadAloudTtsRouter泛化路由）、`16a55b63`（第七步：TTS引擎V2管理界面TtsEngineManageActivity，列表/启停/选为当前引擎/导入冲突处理/导出/删除/恢复经典选择，内置语音包引擎外观行展示）、`dfa82af4`（第八步：选角绑定与全局兜底接入引擎维度，AI角色弹窗两段式选引擎再选音色，旁白/对白池写NG键兼容旧偏好，V2 SYSTEM引擎路由TTSReadAloudService按enginePackage绑定）、`bad35e23`（补录Room数据库112版schema导出：book_tts_voice_bindings加engineId列）、`900ce9e5`（本次编译修复：AI分镜选角发音人可空声明收紧、ReadAloud缺getPrefString导入、HttpReadAloudService缺postEvent导入、AiMultiVoiceDialog缺fetch_empty字符串（默认+中文）、TTS引擎管理页改实现onActivityCreated））。本次编译首轮失败：`compileAppCKotlin` 报 6 处错误——`BdReadAloudService.kt:315` `BdSpeakerRecord?` 实参传给非空形参（`?: record` 已兜底，声明类型误标可空）、`ReadAloud.kt:182` 缺 `io.legado.app.utils.getPrefString` 导入（仅导入了 getPrefBoolean）、`HttpReadAloudService.kt:356` 缺 `postEvent` 导入、`AiMultiVoiceDialog.kt:267` 引用不存在的字符串资源 `tts_engine_v2_fetch_empty`、`TtsEngineManageActivity.kt:44` 未实现 `BaseActivity` 抽象成员 `onActivityCreated`（误写成重写 onCreate）。修复提交 `bad35e23`/`900ce9e5` 后重编译成功。最终一轮：BUILD SUCCESSFUL in 3m 41s（75 actionable tasks: 17 executed, 58 up-to-date），err.log 为空，退出码 0。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.083012c` / `10768`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.083012_10768.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10767` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`、无残留构建进程；未安装到模拟器、未做正式回归。
