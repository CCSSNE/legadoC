...
- `3.26.083118` / `10789`，2026-08-31（UTC 18 时），`own` 主线，**自用版 appC 单编译交付**（编译锚点 `own @ e7fd95c7`；基于基线 `10787`（锚点 `90d5921a`）后新增提交 `4d366abc`（跨页复制触发统一）、`96c81b43`（文档：朗读流程图示更名）、`e7fd95c7`（朗读起点统一落地：双击按裂段段首起读，pageSplit 有效值单一入口，滚动模式锁定关闭））。`-Pabi=arm64-v8a -PVERSION_CODE=10789 -PVERSION_NAME=3.26.083118`，冷编译 `--no-daemon --max-workers=1 -Dorg.gradle.parallel=false -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`。`assembleAppC` BUILD SUCCESSFUL in 2m 16s，`aapt` 确认包名 `io.legado.app.dev`、versionName `3.26.083118c`、arm64-v8a、应用名 `legado C`；产物 `app\build\outputs\apk\app\c\legado_app_3.26.083118_10789.apk`（48.30 MB），备份于 `D:\AI\audio\build-tmp\legado_app_3.26.083118_10789.apk`。`apksigner` 退出码 0，证书 `CN=Android Debug`、SHA-256 `70cb88ca...`（debug 签名，符合只允许 release 系构建 + 统一 debug 签名的签名铁律）。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。旧 10788 中间产物（`legado_app_3.26.083118_10788.apk`，不含 e7fd95c7）已从输出目录清理。
