...
- `3.26.082820c` / `10756`，2026-08-28（UTC 20 时），`own` 主线（编译前锚点 `own @ 4f0bb1e`，`own` 与 `origin/own` 同步；基于基线 `10755` 后新增另一会话的实时缓存提交 `2ca901d`（TTS 实时缓存三态解决 + 任务列表预取）、`4f0bb1e`（单段合成超时看门狗））。用户要求“编译”，按正式 `appC` 编译执行：冷编译一次通过：BUILD SUCCESSFUL in 3m 39s（75 actionable tasks: 20 executed, 55 up-to-date），err.log 为空，退出码 0；构建期间另一会话 Gradle daemon（`C:\Users\user\.gradle`）构建 `D:\AI\audio\android`，非同一工作区无冲突。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082820c` / `10756`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`，即 `C:\Users\user\.android\debug.keystore` 默认调试证书）。产物 `legado_app_3.26.082820_10756.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10755` APK 已删除。构建后 `gradlew --stop`（仅限本构建 Gradle home）、无残留构建进程；未安装到模拟器、未做正式回归。
