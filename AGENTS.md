...
- `3.26.083117` / `10787`，2026-08-31（UTC 17 时），`own` 主线，**自用版 appC 单编译交付**（编译锚点 `own @ 90d5921a`；基于基线 `10786`（锚点 `d3c7db8a`）后新增提交 `eb7da4e3`（记录 appC 单编译交付基线）、`062e4086`（修复 publish-oss-source.ps1 PowerShell 5.1 兼容）、`8990f4c3`（朗读起点规则统一：删除页首按段开关，段中间起读只保留选中朗读）、`90d5921a`（朗读设置耳机按键开关文案写明作用域））。`-Pabi=arm64-v8a -PVERSION_CODE=10787 -PVERSION_NAME=3.26.083117`，冷编译 `--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`。`assembleAppC` BUILD SUCCESSFUL in 4m 37s（75 actionable tasks: 6 executed, 69 up-to-date），`aapt` 确认包名 `io.legado.app.dev`、versionName `3.26.083117c`、arm64-v8a、应用名 `legado C`；产物 `app\build\outputs\apk\app\c\legado_app_3.26.083117_10787.apk`（48.30 MB），备份于 `D:\AI\audio\build-tmp\legado_app_3.26.083117_10787.apk`。`apksigner` 退出码 0，证书 `CN=Android Debug`、SHA-256 `70cb88ca...`（debug 签名，符合只允许 release 系构建 + 统一 debug 签名的签名铁律）。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。旧 10786 产物（`legado_app_3.26.083109_10786.apk`）已从输出目录清理。
