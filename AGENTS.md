...
- 最近一次自用版交付为 `3.26.090320` / `10820`，2026-09-03，基于最新提交 `91abecbf`（修复聚合主页两处编译错误：TabLayout 指示器高度改用公开 setter、排行榜行绑定返回 root 视图）使用 `assembleAppC` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`）。产物包名 `io.legado.app.dev`、versionName `3.26.090320c`、versionCode `10820`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090320_10820.apk`。上一自用版为 `3.26.090308` / `10818`；最近公开版为 `3.26.090318` / `10819`，2026-09-03，使用 `assembleOssRelease` 冷编译成功（`-Pkotlin.compiler.execution.strategy=in-process`）。产物包名 `io.legado.app.c`、versionName `3.26.090318`、versionCode `10819`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C`（label-zh 逐字匹配）、`apksigner` 验证通过；APK 位于 `app\build\outputs\apk\oss\release\legado_oss_3.26.090318_10819.apk`。注：同日先编的 `3.26.090317` / `10805` 因 versionCode 低于线上公开版 `3.26.090304` / `10816` 未发布作废；本轮首编 `10820` 曾因聚合主页两处源码错误失败（`tabIndicatorHeight` 包私有字段访问、`bindRow` 返回绑定而非 View），修复提交后以同一版本重编成功，未另增 versionCode。
