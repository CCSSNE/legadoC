...
- 最近一次自用版交付为 `3.26.090505` / `10842`，2026-09-05，基于提交 `03fb0ece`（自 `10841` 基线后 6 个提交：老记号先读后删、宫格弹窗、脱敏出口、四处换共用预览加保存MD、字符串去重、修括号）使用 `assembleAppC` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，`BUILD SUCCESSFUL in 5m 36s`，err 为空；三次拉起工具调用均报 `ChildProcess.kill` 超时，但 Gradle 子进程存活、out.log 持续增长，属 2026-09-04 已记录现象，未重复拉起）。前两次 `10842` 拉起失败未产出包：一次 `mergeAppCResources` 报中英文宫格字符串重复定义（删重复复用现有），一次 `compileAppCKotlin` 报视频简介分支多余右括号（删一行），均修后同版本重编。产物包名 `io.legado.app.dev`、versionName `3.26.090505c`、versionCode `10842`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过（退出码 0，`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090505_10842.apk`（49720092 字节）。上一自用版为 `3.26.090503` / `10841`；最近公开版为 `3.26.090420` / `10837`，2026-09-04，基于提交 `4a69c948`（README 更新 2026-09-04 记录）使用 `assembleOssRelease` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，计划任务脱离宿主进程树拉起，`BUILD SUCCESSFUL in 7m 15s`，err 为空；任务/启动器事后已删）。产物包名 `io.legado.app.c`、versionName `3.26.090420`（无后缀）、versionCode `10837`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C`（label-zh 逐字匹配）、`apksigner` 验证通过（退出码 0，`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\oss\release\legado_oss_3.26.090420_10837.apk`（34592960 字节），已发布 Release `v3.26.090420`（正式版 draft=false / prerelease=false，资产下载 200）。此前同日编译过 `10836`（versionName 同为 `3.26.090420`，基于 `f4c88c5a`，未发布 Release，产物已被 10837 覆盖删除）。上一公开版为 `3.26.090400` / `10827`（曾创建 Pre 版 Release `v3.26.090400`）。
