...
- `3.26.083119` / `10790`，2026-08-31（UTC 19 时），`own` 主线，**自用版 appC 单编译交付**（编译锚点 `own @ aafb1338`；基于基线 `10789`（锚点 `e7fd95c7`）后新增提交 `df6c51f0`（记录基线 10789）、`9b2c7662`（合集与分组支持改名）、`47468a8d`（合集详情页选择模式支持融合与添加快捷方式，融合逻辑抽为共享入口，快捷方式按钮文案缩减为四字）、`aafb1338`（修复 BookCollectionActivity 缺少 EditText 导入导致编译失败））。`-Pabi=arm64-v8a -PVERSION_CODE=10790 -PVERSION_NAME=3.26.083119`，冷编译 `--no-daemon --max-workers=1 -Dorg.gradle.parallel=false -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`。`assembleAppC` BUILD SUCCESSFUL in 5m，`aapt` 确认包名 `io.legado.app.dev`、versionName `3.26.083119c`、arm64-v8a、应用名 `阅读 C`；产物 `app\build\outputs\apk\app\c\legado_app_3.26.083119_10790.apk`（48.31 MB），备份于 `D:\AI\audio\build-tmp\legado_app_3.26.083119_10790.apk`。`apksigner` 退出码 0，证书 `CN=Android Debug`、SHA-256 `70cb88ca...`（debug 签名，符合只允许 release 系构建 + 统一 debug 签名的签名铁律）。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。首次编译因 `BookCollectionActivity.kt` 缺 `EditText` 导入报 `Unresolved reference` 失败，补导入后同版本重编译成功，失败包未产出。
