...
- `3.26.082415c` / `10733`，2026-08-24（UTC），`own` 主线（基于 `93dd275` 编译；编译前锚点 `own @ 93dd275`，工作区干净，`own` 与 `origin/own` 同步；版本号高于最近有效交付 `10732`）。用户要求“编译，编译”，按正式 `appC` 编译执行：确认无残留 Gradle/Kotlin/Java 进程后，按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10733.bat`，`Start-Process -WindowStyle Hidden`）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`；编译前按规则只删除产物目录中对应旧 APK `legado_app_3.26.082415c_10732.apk`。首轮构建（PID 21104）在 `dexBuilderAppC` 阶段 Gradle daemon native OOM 崩溃（`hs_err_pid25672.log` 与 `replay_pid25672.log`，`Chunk::new` malloc 失败，宿主 31G 内存/swap 不足），非源码错误；按“先停止 Gradle、清理残留进程”规则杀掉崩溃 daemon 的残留 Kotlin 编译器进程 35668、删除崩溃日志后以同一脚本重启（PID 37244）。重启构建 BUILD SUCCESSFUL in 1m 55s，退出码 0（4 executed / 71 up-to-date）；构建期间无新提交，产物对应干净的 `93dd275`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082415c` / `10733`、中文名 `阅读 C`（zh/zh-CN/zh-HK/zh-TW）、架构 `arm64-v8a`、compileSdk/targetSdk 36、`application-debuggable` 与 `10732` 基线一致；`apksigner` 完整执行退出码 0（META-INF 未受签名保护条目提示可接受；注意校验退出码不得经管道截断）。产物原始文件名缺 `c`（`legado_app_3.26.082415_10733.apk`），aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082415c_10733.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建后 `gradlew --stop`，确认无残留 java/kotlin/gradle 进程并清理崩溃遗留 `replay_pid25672.log`；未安装到模拟器、未做正式回归。
