...
别的 Agent 都护着用户数据，我们反着来：AI 相关全部配置一律破坏性升级，今后写 AI 一律沿此开关。版本检测用现成的 `AgentConfig.SCHEMA_VERSION`，改结构就加一，对不上即过期；过期只删旧建新——不校验、不迁移、不兼容，按随包默认与内置种子重建。按零用户设计，不为不存在的用户与数据做任何保护。判错只有三条：设置不对就报错崩溃，错；把旧数据搬进新版本，错；给不存在的用户写保护代码，错。要删就删干净：所有 AI 的旧设置和旧存档一个不留。不管设置烂成什么样，App 必须照常开机。
