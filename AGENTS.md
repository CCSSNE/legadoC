...
- `3.26.081710c` / `10654`，2026-08-17（GMT+8），已正式后台构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；已覆盖安装到雷电模拟器 `emulator-5554` 并完成实际回归。AI 设置分组顺序为「供应商与模型」「日志」「AI 章节净化」「AI 助手」「AI MCP 服务器」「AI 联网工具」「超时」；章节净化条目顺序为「AI章节净化流程说明」「去除广告」「清理噪声」「修正明显错别字」，并保持复用当前模型开关对四个独立配置项的隐藏/恢复联动。普通 AI 请求、章节净化独立请求和连接测试统一读取编辑请求模板；模板不再强制要求 `{{model}}`、`{{systemPrompt}}`、`{{userContent}}`，保存时不做人为校验，用户自定义模板导致的构造或服务端错误按真实结果反馈；连接测试使用最短的 system/user 内容 `ping`。真实回归覆盖：设置顺序、独立配置隐藏/恢复和全程无崩溃。针对性测试通过；全量测试仍有既存无关失败 `ReadBookConfigTest.sanitize_clampsUnsafeLineSpacing`。
