...
- `3.26.082001c` / `10701`，2026-08-20（UTC），`own` 主线（代码提交 `8f336c8`）。书架"返回球"（拖动书籍时屏幕正下方出现的白底蓝边圆球，`root_drop_target`）白底内加两行「放回/主页」字：`fragment_books.xml` 与 `activity_book_collection.xml` 的 `root_drop_target` 由 `View` 改 `TextView`（56dp 圆、accent 蓝 16sp bold、`includeFontPadding=false`、`gravity=center`，样式先经 Frida 注入模拟器实机确认定稿）；新增字符串 `book_drop_target_back_home`（默认 `values/strings.xml` + `values-zh`），`a39edbb`。本次编译还修复了合并进来的 `620d41b` 引入的编译错误：SDK 36 的 `android.text.StaticLayout.Builder` 已移除 `release()`，删除 `AudioPlayActivity.kt` 中 `builder.release()` 调用（`8f336c8`）。已通过正式后台 `:app:assembleAppC` 冷编译（`--no-daemon --max-workers=1` 关增量，第一次因上述 `release()` 报 `Unresolved reference` 失败、修复后同版本重编译），退出码为 0；`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082001c` / `10701`、中文名 `阅读 C`（zh locale）、架构 `arm64-v8a`，`apksigner` 退出码为 0。`own` 本地代码到 `8f336c8` 已提交（未 push）。未安装到模拟器、未做正式回归（用户仅要求编译）。
