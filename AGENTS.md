...
- `3.26.081701c` / `10630`，2026-08-15，已正式构建、APK 校验并安装到雷电模拟器。书源脚本对 `Book.setUseReplaceRule(false)` 的越权调用已被统一拒绝；以该书源新搜索、加入书架并打开正文回归后，书籍配置未被写入 `useReplaceRule=false`，阅读正常进入。
