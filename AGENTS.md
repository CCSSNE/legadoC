...
- `3.26.083021c` / `10773`，2026-08-30（UTC 21 时），`own` 主线（编译锚点 `own @ 0a9a2a64`；基于基线 `10772`（锚点 `6e3d632c`）后新增提交 `0a9a2a64`（AI创作悬浮窗应用内所有非预览界面常驻：BaseActivity统一挂载使各Activity自动获得，创作对话框在自身窗口顶层挂载（非预览页显示）；预览页在前台时全部宿主隐藏，退出预览页即恢复显示；叉叉关闭后下次生成图片重新出现；看书页原挂载逻辑上收删除））。本次编译自动触发（持续监控到 `own` 分支出现新提交 `0a9a2a64` 后自动执行正式编译）。采用冷编译参数 `--no-daemon --max-workers=1` 全量编译成功，BUILD SUCCESSFUL in 2m 57s（75 actionable tasks: 13 executed, 62 up-to-date），err.log 为空，退出码 0。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.083021c` / `10773`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.083021_10773.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10772` APK 已删除。构建后 `gradlew --stop`、按 PID 清理残留 Gradle/Kotlin daemon；未安装到模拟器、未做正式回归。
