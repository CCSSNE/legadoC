...
- `3.26.082911c` / `10761`，2026-08-29（UTC 11 时），`own` 主线（编译锚点 `own @ 4fddab3`，`own` 与 `origin/own` 同步；基于基线 `10760` 后新增提交 `88e9445`（AI设置内置默认供应商 opencode-zen 与模型 big-pickle）、`9cd6621`（正文长按菜单默认不再包含词典与问AI）、`b760831`（AI 创作架构调整：变量定义升为中枢，参数废除合并整块、全部点名引用）、`82f95cd`（正文长按菜单增加编辑入口跳转配置弹窗）、`2d5ffec`（正文长按条编辑支持勾选与拖动排序）、`146f9e5`（AI 创作生图链路：图片 API 配置与请求模板渲染、独立缓存表 v109 迁移）、`d55a898`（AI 创作第三页生图入口与 3.1 预览页、生成任务悬浮窗）、`ee84753`（缓存管理新增创作项：三列宫格展示生成图片，长按多选、全选切换、上传/保存/删除）、`eadaf9d`（AI 设置创作分组新增图片 API 地址/Key/模型/请求模板/重试次数）。编译暴露源码错误 3 处（同一轮），逐一修复后同版本通过：① `CacheManageActivity` 返回键回调 `onBackPressedDispatcher.addCallback(this)` 在 `binding.run` 上下文中 `this` 被解析为 `ActivityCacheManageBinding` 而非 Activity，owner 参数类型不匹配，改为 `this@CacheManageActivity`——提交 `2609222`；② `ReadBookActivity` `aiCreationFloatingView` 未声明成员，补充 `private var aiCreationFloatingView: View? = null`——提交 `3b60d81`；③ `AiCreationDialog` 预览 `PreviewViewHolder` 内 `slots`/`state`/`fileName` 无法解析（`PreviewViewHolder` 是 `AiCreationDialog` 的直接 inner class，无法访问 `PreviewAdapter.slots` 实例属性），改经 `previewAdapter.slots` 取成图列表——提交 `4fddab3`。最终一轮：BUILD SUCCESSFUL in 4m 5s（75 actionable tasks: 9 executed, 66 up-to-date），err.log 为空，退出码 0；构建期间无新提交（编译后新增纯资产提交 `c1c46ea`：提交 Room v109 迁移导出的 schema `109.json`，不参与 APK 输入，未重编译）。失败包未产出、未交付。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082911c` / `10761`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.082911_10761.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10760` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`（仅限本构建 Gradle home）、无残留构建进程；未安装到模拟器、未做正式回归。
