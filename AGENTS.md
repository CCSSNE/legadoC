...
- 最近一次自用版交付为 `3.26.090916c` / `10890`，2026-09-09，基于提交 `b059e5f2`（删除第二节设备与测试边界，含离线底栏失败修复回滚后的干净基线）使用 `assembleAppC` 编译成功（`BUILD SUCCESSFUL in 33s`，`75 actionable tasks: 13 executed, 62 up-to-date`）。产物包名 `io.legado.app.dev`、versionName `3.26.090916c`、versionCode `10890`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner verify` 退出码 0；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090916_10890.apk`，已覆盖安装到无线设备 `10.135.114.254:33465`（`Success`，设备侧确认 `10890`）。
