...
- `3.26.082707c` / `10748`，2026-08-27（UTC），`own` 主线（编译前锚点 `own @ 10f6572`，`own` 与 `origin/own` 同步；基于基线 `10747` 后新增提交 `6f21ad0`（删除记录调试日志开关，模块勾选即记录入口）、`10f6572`（修复 putDebug 丢失 module 参数导致编译失败）。用户要求“编译”，按正式 `appC` 编译执行：22 个 stat 缓存脏标记文件经 blob 对比确认与 HEAD 完全一致，无残留构建进程，按内存压力规则冷编译 `:app:assembleAppC`。后台 `.bat`（`build_10748.bat`，回读校验 14 行后 `Start-Process -WindowStyle Hidden`）日志与退出码写入 `D:\AI\audio\build-tmp`。第一次编译在 `compileAppCKotlin` 失败（2m 6s）：提交 `6f21ad0` 重写 `putDebug` 时删掉 `module` 参数，但 `57bf03a` 布下的 12 处调用点仍传 `module = LogModule.READ_ALOUD`；恢复参数并转发给 `put()` 后提交 `10f6572` 推送，同版本重编译一次通过：BUILD SUCCESSFUL in 4m 37s（75 actionable tasks: 7 executed, 68 up-to-date），err.log 为空，退出码 0；重编译期间无新提交。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082707c` / `10748`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`，内部元数据正确，按规则改名为 `legado_app_3.26.082707c_10748.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`；删除同目录旧 `10747` APK 与构建生成的未跟踪 `app\schemas\...\107.json`。构建后 `gradlew --stop`（无 daemon）、无残留构建进程；未安装到模拟器、未做正式回归。
