...
- 最近一次自用版交付为 `3.26.090520` / `10848`，2026-09-05，基于提交 `9e282c04`（自 `10847` 基线后 5 个提交：`191a3bb4` 工作流补记LLM返回、`01a5ee35` 工作流删顶层prompt、`eb204bff` 上框记最后一次实际发送、`bd877e0a` 回退上框记最后一次发送、`9e282c04` 上框快照规则钉注释）使用 `assembleAppC` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，第三轮因编译期间新增提交已删 `app/build` 全量重编，`BUILD SUCCESSFUL in 5m`，`75 actionable tasks: 21 executed, 20 from cache, 34 up-to-date`，err 为空仅有编译警告）。产物包名 `io.legado.app.dev`、versionName `3.26.090520c`、versionCode `10848`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过（退出码 0，`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090520_10848.apk`（49739424 字节）。上一自用版为 `3.26.090518` / `10847`；最近公开版为 `3.26.090420` / `10837`，2026-09-04，基于提交 `4a69c948`（README 更新 2026-09-04 记录）使用 `assembleOssRelease` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，计划任务脱离宿主进程树拉起，`BUILD SUCCESSFUL in 7m 15s`，err 为空；任务/启动器事后已删）。产物包名 `io.legado.app.c`、versionName `3.26.090420`（无后缀）、versionCode `10837`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C`（label-zh 逐字匹配）、`apksigner` 验证通过（退出码 0，`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\oss\release\legado_oss_3.26.090420_10837.apk`（34592960 字节），已发布 Release `v3.26.090420`（正式版 draft=false / prerelease=false，资产下载 200）。此前同日编译过 `10836`（versionName 同为 `3.26.090420`，基于 `f4c88c5a`，未发布 Release，产物已被 10837 覆盖删除）。上一公开版为 `3.26.090400` / `10827`（曾创建 Pre 版 Release `v3.26.090400`）。
