...
- `3.26.082623c` / `10740`，2026-08-26（UTC），`own` 主线（编译前锚点 `own @ 883829a`，工作区干净，`own` 与 `origin/own` 同步；基于基线 `10739` 后新增提交 `7216e09`（页首按段开关迁至朗读设置并明确命名与注释）、`2b37814`（页首按段与按页朗读解耦）、`49dca0b`（按页朗读开启时隐藏原进度/从本页读面板）、`28fd7e1`（从本页读跨页段维持本页）、`883829a`（朗读高亮按全局段落号标记并改主线程更新）编译，UTC 已进入 08-26 23 时，版本名为 `3.26.082623`，10740 > 10739）。用户要求“编译”，按正式 `appC` 编译执行：核查无残留 Gradle/Kotlin/Java 进程、无 `hs_err_pid*`/`replay_pid*` 崩溃日志（内存余量 ~14GB），并按规则只删除产物目录中对应旧 APK `legado_app_3.26.082500c_10739.apk`；随后按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10740.bat`，字面量脚本回读校验 12 行后 `Start-Process -WindowStyle Hidden`，PID 42332）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`。编译一次通过：BUILD SUCCESSFUL in 2m 23s（76 actionable tasks: 20 executed, 56 up-to-date），err.log 为空，退出码 0；构建期间无新提交，产物对应干净的 `883829a`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082623c` / `10740`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`（`legado_app_3.26.082623_10740.apk`），aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082623c_10740.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建产物 `app\schemas\...\106.json`/`107.json`（copyRoomSchemas 生成、未跟踪，本轮无数据库版本变更）按 569dc4a“Room 构建时重新生成”先例删除。构建后 `gradlew --stop`（无 daemon），确认无残留构建进程与崩溃日志；未安装到模拟器、未做正式回归。
