...
- `3.26.082914c` / `10762`，2026-08-29（UTC 14 时），`own` 主线（编译锚点 `own @ 3343aee`，`own` 与 `origin/own` 同步；基于基线 `10761` 后新增提交 `7b61014`（AI分角色数据层：Room v110 新增正式角色/临时角色/音色绑定三表与迁移、分镜缓存文件存储与按书自动化开关）、`82c9093`（百度离线TTS修复初始化必败：装配表从未构建导致 reloadModel 恒返 -1，改为适配器直接装配路径语义；init 链补数据文件缺失检查；试听透出真实失败原因）、`28ab629`（BdReadAloudService 通知 PendingIntent 统一走 servicePendingIntent 入口，修复 S+ 缺 FLAG_MUTABLE 导致创建朗读通知出错、服务自杀）、`4ee9eff`（AI分角色核心：移植分镜归因 Helper 与选角 Coordinator，严格校验+二分重试+临时角色收编+自动选音）、`ded55b0`（AI分角色合成接入：百度TTS朗读服务接入分镜驱动，缓存优先+段内偏移求交+角色音色路由+兜底池回退+后台预生成）、`f86b263`（AI分角色UI：朗读弹窗插入AI角色按钮，新增两页总弹窗、角色编辑/批量分析/分镜缓存弹窗，AI设置新增分角色分组）、`8907fbb`（正文长按条暂存动作）、`9cc1b11`（AI 创作选中文本分区作用域默认改一次性）、`ed30afe`（百度TTS调试日志：新增BAIDU_TTS日志模块与引擎初始化/合成/试听/语音包导入打点））。首次正式编译暴露源码错误 10 类（同一轮），补齐缺失 import 并修正一处类型使用后同版本通过——提交 `df17de3`：① `AiStoryboardConfig` 缺 `splitties.init.appCtx`；② `AiBatchAnalyzeDialog`/`AiStoryboardCacheDialog` 缺 `AiStoryboardConfig` import；③ `AiStoryboardBatchAnalyzer` 缺 `kotlinx.coroutines.isActive`；④ `AiTtsStoryboardHelper`/`BookTtsCastingCoordinator`（也说 AppConfig 同款用法）缺 `io.legado.app.utils.fromJsonArray` 扩展 import；⑤ `BookTtsCastingCoordinator` 缺 `com.google.gson.JsonObject`；⑥ `StoryboardModels` 缺 `BookRole` import；⑦ `BdReadAloudService` 局部 `textChapter` 是 `TextChapter`，`chapter.index` 须改为 `chapter.chapter.index`（6 处）；⑧ `BdReadAloudService` 缺 `androidx.lifecycle.lifecycleScope`；⑨ `AiRoleEditDialog` 缺 `fromJsonArray`；⑩ `AiConfigFragment` 缺 `showIntegerInputDialog`。编译后新增纯资产提交 `3343aee`（Room v110 迁移导出 schema `110.json`，不参与 APK 输入，未重编译）。最终一轮：BUILD SUCCESSFUL in 5m 22s（75 actionable tasks: 9 executed, 66 up-to-date），err.log 为空，退出码 0；构建期间无新提交。首轮 BUILD FAILED（`compileAppCKotlin` 报上述源码错误，44 tasks），失败包未产出、未交付。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082914c` / `10762`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.082914_10762.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10761` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`（仅限本构建 Gradle home）、无残留构建进程；未安装到模拟器、未做正式回归。
