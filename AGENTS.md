...
- `3.26.081619c` / `10624`，2026-08-15，已安装到雷电模拟器；按用户要求仅完成构建、APK 校验和覆盖安装，未额外进行功能回归。默认主题背景初始化改为分别复制随 APK 提供的 `bg/护眼漫绿.jpg` 与 `bg/宁静夜色.jpg` 到应用私有目录，再写入日间 `backgroundImage`、夜间 `backgroundImageNight`；已配置或明确清空的偏好不会被覆盖。
