...
- `3.26.082706c` / `10747`，2026-08-27（UTC），`own` 主线（编译前锚点 `own @ 57bf03a`，`own` 与 `origin/own` 同步；基于基线 `10746` 后新增提交 `04bde27`（普通日志按模块过滤）、`57bf03a`（朗读诊断日志统一 `[朗读]` 前缀归朗读模块），UTC 仍为 08-27 06 时）。用户要求“编译”，按正式 `appC` 编译执行：核查无残留构建进程后按内存压力规则冷编译 `:app:assembleAppC`。后台 `.bat`（`build_10747.bat`，字面量 here-string 回读校验 11 行后 `Start-Process -WindowStyle Hidden`，PID 20728）日志与退出码写入检出目录外 `D:\AI\audio\build-tmp`。编译一次通过：BUILD SUCCESSFUL in 5m 18s（76 actionable tasks: 21 executed, 55 up-to-date），err.log 为空，退出码 0；构建期间无新提交。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082706c` / `10747`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`，内部元数据正确，按规则改名为 `legado_app_3.26.082706c_10747.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建产物 `app\schemas\...\106.json`/`107.json`（copyRoomSchemas 生成、未跟踪）按先例删除。构建后 `gradlew --stop`（无 daemon）；未安装到模拟器、未做正式回归。
