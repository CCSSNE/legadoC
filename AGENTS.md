...
- `3.26.082500c` / `10739`，2026-08-25（UTC），`own` 主线（编译前锚点 `own @ b8f6098`，工作区干净，`own` 与 `origin/own` 同步；基于基线 `10738` 后新增提交 `b8f6098 feat(bookshelf): place shortcuts in collections` 编译，UTC 已进入 08-25 00 时，版本名为 `3.26.082500`，10739 > 10738）。用户要求“编译”，按正式 `appC` 编译执行：核查无残留 Gradle/Kotlin/Java 进程、无 `hs_err_pid*`/`replay_pid*` 崩溃日志（内存余量 ~10.5GB），并按规则只删除产物目录中对应旧 APK `legado_app_3.26.082423c_10738.apk`；随后按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10739.bat`，字面量脚本回读校验 10 行后 `Start-Process -WindowStyle Hidden`，首轮 PID 39748）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`。首次编译在 `compileAppCKotlin` 失败（BUILD FAILED in 1m 30s），err.log 给出一处源码错误：`BookShortcutHelp.kt:46:67` `'toShelfBook' is a member and an extension at the same time. References to such elements are prohibited.`——`toShelfBook` 是 `object BookShortcutHelp` 内部的 `BookShortcutWithBook` 扩展成员（135 行），39/56 行以 `item.toShelfBook(...)` 直接调用合法可编译，唯独 46 行 `?.let(item::toShelfBook)` 对“既是对象成员又是扩展”的名字取可调用引用被 Kotlin 明确禁止；`b8f6098` 提交引入。修复为 lambda 调用 `?.let { item.toShelfBook(it) }`，以中文消息提交 `d19a50a` 并推送；按 2026-08-15 先例失败未交付同版本重编译，锚点 `own @ d19a50a`。重编译（PID 40536）BUILD SUCCESSFUL in 4m 45s（8 executed / 68 up-to-date），err.log 为空，退出码 0；构建期间无新提交，产物对应干净的 `d19a50a`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082500c` / `10739`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36、`application-debuggable`；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`（`legado_app_3.26.082500_10739.apk`），aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082500c_10739.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建产物 `app\schemas\...\106.json`/`107.json`（copyRoomSchemas 生成、未跟踪，107 为 `b8f6098` 数据库版本变更后新增）按 569dc4a“Room 构建时重新生成”先例删除。构建后 `gradlew --stop`（无 daemon），确认无残留构建进程与崩溃日志；未安装到模拟器、未做正式回归。
