...
- `3.26.082900c` / `10758`，2026-08-29（UTC 0 时），`own` 主线（编译前锚点 `own @ 4d733e5`，`own` 与 `origin/own` 同步；基于基线 `10757` 后新增提交 `4f43151`（在线(HTTP)引擎批量缓存单元级多线程合成，产物后缀按引擎区分 mp3/wav）及编译错误修复提交 `4d733e5`）。用户要求继续监控新提交自动编译：按分钟轮询至 `4f43151` 落库且工作区干净后触发正式 `appC` 编译。首次冷编译在 `:app:compileAppCKotlin` 失败：该提交重写 `TtsCacheManager.kt`/`TtsCacheStore.kt` 时再次丢失 3 个扩展导入（`simulatedTotalChapterNum`/`flow.collect`/`fromJsonObject`，与 `10755` 首编失败同根因），补齐后提交 `4d733e5` 推送、同版本重编一次通过：BUILD SUCCESSFUL in 4m 26s（75 actionable tasks: 7 executed, 68 up-to-date），err.log 为空，退出码 0；构建期间无新提交。失败包未产出、未交付。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082900c` / `10758`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.082900_10758.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10757` APK 已删除。构建后 `gradlew --stop`（仅限本构建 Gradle home）、无残留构建进程；未安装到模拟器、未做正式回归。经验教训：`TtsCacheManager.kt`/`TtsCacheStore.kt` 已两次因重写丢失文件内扩展导入导致编译失败，改动这两个文件时必须复查 `simulatedTotalChapterNum`、`kotlinx.coroutines.flow.collect`、`fromJsonObject` 三个导入仍在。
