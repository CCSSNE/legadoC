...
- `3.26.083101c` / `10775`，2026-08-31（UTC 01 时），`own` 主线（编译锚点 `own @ 35b75852`；基于基线 `10774`（锚点 `1df084bf`）后新增提交 `5960f474`（朗读引擎选择行编辑/删除按钮整体左移5dp：删除按钮避开右上角飘带红条，编辑按钮随链式约束同步平移间距保持为零）、`0132dfca`（V2朗读引擎默认内置收敛为仅MultiTTS转发器：删除脚本选项示例与内置发音人示例两个模板资产，DEFAULT_SCRIPT_ASSETS与注释同步收敛）与 `35b75852`（补录删除两个模板资产文件，使工作区与HEAD一致））。本次编译自动触发（持续监控到 `own` 分支出现新提交 `5960f474`、`0132dfca` 后自动执行正式编译；编译前发现 `0132dfca` 的工作区仍残留两个资产文件删除未提交，按不可变交付约束先行补录提交 `35b75852` 后再编译）。采用冷编译参数 `--no-daemon --max-workers=1` 全量编译成功，BUILD SUCCESSFUL in 2m 16s（75 actionable tasks: 22 executed, 53 up-to-date），err.log 为空，退出码 0。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.083101c` / `10775`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.083101_10775.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10774` APK 已删除。构建后 `gradlew --stop`、按 PID 清理残留 Gradle/Kotlin daemon；未安装到模拟器、未做正式回归。
