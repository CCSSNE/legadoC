...
- `3.26.083109` / `10786`，2026-08-31（UTC 09 时），`own` 主线，**自用版 appC 单编译交付**（编译锚点 `own @ d3c7db8a`；基于基线 `10785`（锚点 `c31eae97`）后新增提交 `cbaa772d`（TTS缓存入口不可用不再静默：置灰保持可点，点击弹底部通知说明原因）、`d3c7db8a`（TTS缓存门控统一：不可用原因收敛到TtsCacheParams唯一源头，朗读面板/沉浸页下载按钮/选章弹窗同源引用，沉浸页本地书下载语义不再限定系统TTS引擎））。`-Pabi=arm64-v8a -PVERSION_CODE=10786 -PVERSION_NAME=3.26.083109`，冷编译 `--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`。`assembleAppC` BUILD SUCCESSFUL in 3m 3s（75 actionable tasks: 20 executed, 55 up-to-date），`aapt` 确认包名 `io.legado.app.dev`、versionName `3.26.083109c`、arm64-v8a、应用名 `legado C`；产物 `app\build\outputs\apk\app\c\legado_app_3.26.083109_10786.apk`（48.31 MB），备份于 `D:\AI\audio\build-tmp\legado_app_3.26.083109_10786.apk`。`apksigner` 退出码 0，证书 `CN=Android Debug`、SHA-256 `70cb88ca...`（debug 签名，符合只允许 release 系构建 + 统一 debug 签名的签名铁律）。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。旧 10785 产物（`legado_app_3.26.083107_10785.apk`）已从输出目录清理。
