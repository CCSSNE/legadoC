...
- `3.26.081534c` / `10613`，2026-08-15，已在雷电模拟器安装。默认弹窗透明度与模糊度统一为 `70`，全局界面透明度为 `100`，阅读页透明背景默认开启；用户选定的 Pre 背景图已随 APK 内置，冷启动实测日间与夜间均安装到应用私有路径并与源图 SHA-256 一致。显式移除背景以空字符串保存，后续启动不会恢复默认图。
