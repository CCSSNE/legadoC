...
- `3.26.082413c` / `10728`，2026-08-24（UTC），`own` 主线（基于 `23c0fd8` 编译；版本号在先前交付 `10727` 之上单调递增）。用户要求“新提交一到就编译”并持续监控 `own` 分支：首轮编译（HEAD `f1dc307`）暴露 3 个 Kotlin 编译错误，已分别提交 `8a23afa`（补 `ReadBook.kt` 缺失的 `kotlinx.coroutines.isActive` 导入，`while (isActive)` 预下载循环用）与 `f1dc307`（`CacheCoordinator.submitTextDownload`/`submitAutomaticTextDownload` 章节目录形参由 `Collection<Int>` 放宽为 `Iterable<Int>`，收敛 `IntRange` 实参，修 `CacheActivity.kt:277`、`BaseReadBookActivity.kt:300` 类型不匹配）并推送；随后分支出现新提交 `1876fe6`（默认关闭自动评论下载）、`c9a5c0a`（本地 TXT-ZIP 原档删除）、`23c0fd8`（导入媒体不入缓存），按交付规则基于最新分支头 `23c0fd8` 重新编译（构建期间无新提交）。按本机内存压力规则采用冷编译参数（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat` 隐藏窗口启动（`Start-Process -WindowStyle Hidden`），stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`；退出码记录改用 `(echo %EXITCODE%)>` 写法——`echo %EC%>` 无空格会被 cmd 解析为文件描述符重定向，写入内容变成 “ECHO is off.”。BUILD SUCCESSFUL in 6m 26s，退出码 0；err 流为 Kotlin 编译器内部断言与弃用警告（含 `VideoPlay.VIDEO_PREF_NAME` 常量求值断言、`BackupConfigFragment`/`ConfigActivity`/`MainActivity` 弃用提示），非失败。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082413c` / `10728`、中文名 `阅读 C`（zh，含 zh-CN/zh-HK/zh-TW）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（META-INF 未受签名保护条目提示可接受）。产物文件名原为 `legado_app_3.26.082413_10728.apk`，内部元数据正确，仅改名补 `c` 为 `legado_app_3.26.082413c_10728.apk`；旧 `10727` APK 已不在产物目录。收尾 `gradlew --stop`，无残留 java/Gradle/kotlin 进程。持续监控仍在运行：下一锚点为 `23c0fd8`（本轮编译 HEAD），新提交出现将基于新 HEAD 重编译；未安装到模拟器、未做正式回归。注：随后针对 `90220df` 的 `10729` 构建（BUILD SUCCESSFUL、aapt/apksigner 校验通过）因另一会话在 Kotlin 编译阶段（15:05–15:09）就地修改了 14 个源码文件、产物对应不确定混合状态，判定无效未交付（有效交付基线仍为 `10728`）。
