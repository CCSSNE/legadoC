...
- 代码修改完成后，只要改动能够可靠映射为运行时 Hook，默认允许对当前已运行的雷电模拟器应用使用 Frida hot-patch 做针对性验证；这属于开发期动态注入验证，不等同于正式编译、安装或正式回归，也不得据此自行启动 Gradle。用户明确禁止动态验证时不执行。
...
- 用户明确要求验证或交付代码改动时，只能使用正式 `appC` 变体：`app\build\outputs\apk\app\c`。禁止以中间 Gradle 任务、debug APK 或改名旧包充当验证/交付物。
...
- 调试工具链由 Codex 持续运行和采集，包括显式目标的 ADB、uiautomator2、截图、logcat，以及证据不足时按同一次复现升级的 Perfetto、Winscope 和 Frida。
...
- Frida 分为 `trace` 与 `hot-patch` 两种用途。`trace` 用于观察真实运行时对象和调用链；只有常规证据不足，且明确怀疑时序、线程、Window/Surface 合成或运行时调用链时，才升级到 Perfetto、Winscope 或 Frida trace。
- `hot-patch` 是代码修改后的开发期动态验证手段，不受上一条“证据不足才升级”的限制。只要本次 Java/Kotlin 改动能可靠映射到运行时行为，就应优先把对应逻辑注入当前运行进程验证，而不是为了试错启动正式编译。
- hot-patch 可在最小必要范围内替换已有方法实现、修改参数/字段/返回值，并可在确有必要时加载临时 DEX；允许反复 reload 脚本快速迭代。所有注入必须只针对本次待验证行为，禁止顺带改变无关业务状态。
- 资源/XML、Manifest、Gradle 配置、native 代码、签名/打包行为，以及新增/删除类、字段、方法或改变继承结构等无法由运行时 Hook 可靠等价表达的改动，不得宣称已被 Frida 完整验证；应明确指出验证边界，且不得因此自行触发正式编译。
- Frida 只能连接已按第 2 节规则确认的雷电模拟器目标，不固定假设 ADB 端口或 transport；目标不明确时停止。脚本错误、`Java is not defined`、目标类/重载解析失败或注入未实际命中均视为验证失败。
...
- Frida 验证结束后卸载注入脚本、恢复进程原行为并移除模拟器临时 server；hot-patch 只证明运行时实验结果，最终实现仍必须落回正式源码。
