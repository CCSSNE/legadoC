...
- `3.26.082423c` / `10737`，2026-08-24（UTC），`own` 主线。用户要求“编译这个项目”，按正式 `appC` 编译执行。编译前先执行 AGENTS.md 新增 Git 规则（提交必须中文说明、历史英文提交顺手改中文并强推）：以 `msg_filter.py`（Python 字典按首行标题匹配、保留多行正文、未命中原样输出）经 `git filter-branch -f --msg-filter` 将 177 个纯英文提交消息改写为中文，改写后树哈希与改写前一致（`b9e2e812c5cfd2a5a5a696a0b0791775fc2ddf80`）、提交数 668 不变、纯英文消息清零，删除 `refs/original` 备份后 `git push --force-with-lease origin own` 强推，`own` 与 `origin/own` 同步。首次 `assembleAppC` 冷编译在 `compileAppCKotlin` 失败（BUILD FAILED in 2m 28s），err.log 给出 4 处源码错误：`ReviewSnapshotCapture.kt:1207` `futures.forEach { it.cancel(true) }`——`futures` 是 `List<Pair<ResourceTarget, Future>>`（1184 行 `target to executor.submit(...)`），`it` 是 Pair 无 `cancel`，改为解构 `(_, future) -> future.cancel(true)`；`ReviewSnapshotResourceStore.kt:145` 字符串续行 `+` 写在行首被 Kotlin 解析为 `unaryPlus`，改为上一行末尾续行；`BookGroupSelectDialog.kt:120` `shortcutIds` 是 `LongArray` 无 `mapNotNull` 扩展（项目内无自定义扩展，115 行 `bookUrls` 是 `ArrayList` 故可编译），改为 `toList()`，同时消除 `shortcutId` 类型推断失败（`BookShortcutDao.get(shortcutId: Long)` 签名匹配）。修复以中文消息提交 `26aa310`（“修复 appC 编译错误：Future 取消解构、字符串拼接续行、LongArray mapNotNull”）；按 2026-08-15 先例失败未交付同版本重编译，锚点 `own @ 26aa310`，工作区干净、`own` 与 `origin/own` 同步。正式重编译沿用冷编译参数 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10737_r2.bat`，字面量脚本回读校验 10 行后 `Start-Process -WindowStyle Hidden`，PID 11456）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`。BUILD SUCCESSFUL in 4m 45s（10 executed / 66 up-to-date），err.log 为空，退出码 0；构建期间无新提交，产物对应干净的 `26aa310`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082423c` / `10737`、中文名 `阅读 C`（zh/zh-CN/zh-HK/zh-TW）、架构 `arm64-v8a`、compileSdk/targetSdk 36、`application-debuggable`；`apksigner` 完整执行退出码 0（META-INF 未受签名保护条目提示可接受）。产物原始文件名缺 `c`（`legado_app_3.26.082423_10737.apk`），aapt 确认内部版本名正确后按“仅文件名少 c 直接改名”规则改名为 `legado_app_3.26.082423c_10737.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`。构建产物 `app\schemas\...\106.json`（copyRoomSchemas 生成、未跟踪）按 569dc4a“Room 构建时重新生成”先例删除。构建后 `gradlew --stop`（无 daemon），确认无残留构建进程与崩溃日志；未安装到模拟器、未做正式回归。
