...
- 最近一次自用版交付为 `3.26.090418` / `10835`，2026-09-04，基于提交 `ec19fc13`（修复AI创作预览页返回跳到不存在的page2导致空白异常）使用 `assembleAppC` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，`BUILD SUCCESSFUL in 4m 42s`，err 为空；直接 `Start-Process` 拉起被宿主杀掉（日志停在启动横幅、无 java 进程），改用计划任务脱离宿主进程树拉起后构建正常）。产物包名 `io.legado.app.dev`、versionName `3.26.090418c`、versionCode `10835`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过（退出码 0，`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090418_10835.apk`（48479415 字节）。上一自用版为 `3.26.090416` / `10834`；最近公开版为 `3.26.090420` / `10836`，2026-09-04，基于提交 `f4c88c5a`（AI创作：存量参数值升级时全量消毒重置默认值，读取时失效值回落默认不再崩溃）使用 `assembleOssRelease` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，计划任务脱离宿主进程树拉起，`BUILD SUCCESSFUL in 8m 43s`，err 为空；任务/启动器事后已删）。产物包名 `io.legado.app.c`、versionName `3.26.090420`（无后缀）、versionCode `10836`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C`（label-zh 逐字匹配）、`apksigner` 验证通过（退出码 0，`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\oss\release\legado_oss_3.26.090420_10836.apk`（34592589 字节）。上一公开版为 `3.26.090400` / `10827`（曾创建 Pre 版 Release `v3.26.090400`）。
