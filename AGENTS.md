...
- `3.26.090105` / `10794`，2026-09-01（UTC 05 时），`own` 主线，**公开版 oss 单编译交付**（编译锚点 `own @ 68c10c0f`，即基线 `10793` 后新增提交 `4c0933c1`（TXT-ZIP 导出导入支持高亮规则外挂数据）与 `68c10c0f`（补录 Room 113 版 schema 导出））。本次为用户点名"编译公开版"，确认按新版本交付（versionCode 递增 10794）。`-Pabi=arm64-v8a -PVERSION_CODE=10794 -PVERSION_NAME=3.26.090105`，冷编译 `--no-daemon --max-workers=1 -Dorg.gradle.parallel=false -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`。`assembleOssRelease` BUILD SUCCESSFUL in 5m 33s，`aapt` 确认包名 `io.legado.app.c`、versionName `3.26.090105`（无后缀）、versionCode `10794`、arm64-v8a、应用名 `阅读`（繁中 `閱讀`）；产物 `app\build\outputs\apk\oss\release\legado_oss_3.26.090105_10794.apk`（34.74 MB），备份于 `D:\AI\audio\build-tmp\legado_oss_3.26.090105_10794.apk`。`apksigner` 退出码 0，证书 `CN=Android Debug`、SHA-256 `70cb88ca...`（debug 签名，符合只允许 release 系构建 + 统一 debug 签名的签名铁律）。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。沿用此前批次设定的 `GRADLE_USER_HOME=D:\AI\audio\android-gradle-user-home`（与项目同盘），未再触发 KSP 跨盘 bug。
