...
- `3.26.081702c` / `10646`，2026-08-17（GMT+8），已正式后台构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；本次雷电模拟器 `emulator-5554` 离线，未安装和回归。章节净化现按 B 段落逐段执行可编辑的 B→C 预处理，保留段落 ID，完成预处理后再添加编号；广告返回段号，错别字和噪声通过 C→B 映射生成规则。AI 设置新增默认开启的 API 脱敏开关，关闭前显示开发调试警告；关闭后供应商、MCP、Tavily 的 API Key 输入框显示明文，AI 请求/响应头日志保留原始敏感值。章节净化请求、日志、超时和实时状态能力保持上一基线行为。单元测试 `AiChapterPurifyHelperTest` 已通过。
