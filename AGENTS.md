...
- `3.26.090208` / `10805`，2026-09-02（UTC 08 时），`own` 主线，**自用版代理编译交付**。本次为用户点名"编译自用版本"。`-Pabi=arm64-v8a -PVERSION_CODE=10805 -PVERSION_NAME=3.26.090208`，冷编译（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`，`GRADLE_OPTS=-Xmx4096m`）`assembleAppC`，BUILD SUCCESSFUL in 3m 39s（75 actionable tasks: 20 executed, 55 up-to-date），err.log 为空，退出码 0。`aapt` 确认包名 `io.legado.app.dev`、versionName `3.26.090208c`、versionCode `10805`、架构 `arm64-v8a`、compileSdk 36；`apksigner` 退出码 0，证书 `CN=Android Debug`、SHA-256 `70cb88ca...`（debug 签名，符合签名铁律）。产物 `app\build\outputs\apk\app\c\legado_app_3.26.090208_10805.apk`（约 47.6 MB）。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。上一公开版 oss 交付为 `3.26.090202` / `10803`（v3.26.090202，见 Git/GitHub Release 记录）。
