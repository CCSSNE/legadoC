...
- `3.26.081714c` / `10658`，2026-08-17（GMT+8），已正式后台构建并完成 `aapt` / `apksigner` 校验，包名为 `io.legado.app.c`、架构为 `arm64-v8a`；已覆盖安装到雷电模拟器 `emulator-5554` 并完成实际回归。在 10657 基础上：手动编辑 / 反转内容不再只更新已有已完成记录的指纹，而是**无条件把该章标记为已处理**——从未处理或曾处理失败的章也生效（转为 COMPLETED、指纹=编辑后缓存原文 SHA-256、已处理类型并入编辑时启用的全部净化功能），此后任何自动 run 都跳过它、绝不为它生成替换规则（编辑是受控改动，不再被自动重算、不再把正文塞进规则库）；之后用户新启用净化功能时该章因类型未覆盖仍按常规判定重处理。模拟器回归：无记录章打开书自动处理（FAILED 反例）、反转内容后记录转 COMPLETED 且规则库数量不变、冷启动再开书双章 SKIP_COMPLETED 且零 AI 调用。上一交付 10657：AI 章节净化引入「章节处理记录」状态机（`processedTypes` 列 DB 102→103、指纹=缓存原文 SHA-256、force 只作用起始章修复刷新连坐、清除缓存/目录更新/移除书架删记录重算）、AI 思考打断默认值五秒、AI 报错信息中文第一句分类。版本号后续以此基线递增。
