...
- `3.26.082922c` / `10765`，2026-08-29（UTC 22 时），`own` 主线（编译锚点 `own @ 56eed50`，`own` 与 `origin/own` 同步；基于基线 `10764` 后新增提交 `e37cc1f`（收纳AI创作作用域设置）、`c14dc17`（AI分角色新增复用当前AI模型开关：默认开启并隐藏独立分镜供应商/模型项，模型解析与未配置提示对齐AI创作和章节净化）、`494da56`（本地百度TTS页头部接入统一状态栏插区，标题与导入按钮不再与状态栏重叠）、`e48bdec`（修复百度TTS 90013/9294715 引擎 license 必报 -1：验证调用改经引擎实例分发）、`f4cf76f`（AI创作后台生图改为多任务并行：新请求不阻塞也不打断老任务，展示与提示以最新任务为准；老任务后台继续跑完，返回的图照常落盘入库，报错与过程提示静默；文件名收敛到saveBytes单点用时间戳+自增序号保证并发不撞名）、`0b59e32`（AI创作新增图片测试偏好键常量）、`56eed50`（修复AI分角色听书分镜配置缺 `getPrefBoolean`/`putPrefBoolean` 导入））。首轮编译失败：`compileAppCKotlin` 报 `AiStoryboardConfig.kt` 中 `Unresolved reference 'getPrefBoolean'/'putPrefBoolean'`——该文件（源自 `c14dc17`）import 块只导入 Int/String 四个扩展函数，缺 Boolean 两个版本；函数本就在 `ContextExtensions.kt:161/175` 且签名匹配，补 `import io.legado.app.utils.getPrefBoolean` 与 `putPrefBoolean` 后同版本重编译。最终一轮：BUILD SUCCESSFUL in 4m 48s（75 actionable tasks: 7 executed, 68 up-to-date），err.log 为空，退出码 0；构建期间无新提交。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082922c` / `10765`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.082922_10765.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10764` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`、无残留构建进程；未安装到模拟器、未做正式回归。
