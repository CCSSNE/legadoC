...
- `3.26.082723c` / `10753`，2026-08-27（UTC），`own` 主线（编译前锚点 `own @ 5962dda`，`own` 与 `origin/own` 同步；基于基线 `10752` 后新增提交 `7c12183`/`0a93230`/`5962dda`（schema 与截图收录、README 更新记录、朗读面板线程崩溃修复）。用户要求“编译”，按正式 `appC` 编译执行：stat 缓存脏标记文件经 `git diff --stat` 空输出确认与 HEAD 完全一致，无残留构建进程，按内存压力规则冷编译 `:app:assembleAppC`。后台 `.bat`（`build_10753.bat`，回读校验后 `Start-Process -WindowStyle Hidden`）日志与退出码写入 `D:\AI\audio\build-tmp`。编译一次通过：BUILD SUCCESSFUL in 1m 59s（75 actionable tasks: 13 executed, 62 up-to-date），err.log 为空，退出码 0；构建期间无新提交。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082723c` / `10753`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物原始文件名缺 `c`，内部元数据正确，按规则改名为 `legado_app_3.26.082723c_10753.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10752` APK 已删除。构建后 `gradlew --stop`（无 daemon）、无残留构建进程；未安装到模拟器、未做正式回归。
