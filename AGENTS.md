...
- `3.26.082917c` / `10763`，2026-08-29（UTC 17 时），`own` 主线（编译锚点 `own @ 3abac75`，`own` 与 `origin/own` 同步；基于基线 `10762` 后新增提交 `a1e38a2`（AI 创作一次性素材随界面关闭销毁：关闭创作界面即清空临时分区卡片并从会话摘除，应用启动兜底清理进程遗留）、`848d742`（AI创作生图三级回退：批量n请求→直接并发(每批3路,不带重试)→串行带重试兜底；部分返回不再误判为整体失败）、`2e5416a`（删除内置AI供应商默认请求头：不再随默认配置注入opencode伪装头，请求头能力保留由用户自配）、`895e765`（AI设置图片API Key下新增获取智谱API Key入口，点击跳转 bigmodel.cn/apikey/platform）、`0ad5fc6`（跨页复制修复：翻页前本页选区文本先入累计缓冲，复制时拼接已翻过页与当前页选区，不再丢失之前页内容）、`d943e50`（修复百度TTS服务致命缺陷：cancelled闩死致全程无声、朗读位置恒发布0；新增失败章闩防逐段重试风暴、播放器错误兜底推进、收编选音移入IO线程、对白合成去引号）、`2cae9f0`（修复AI分角色收编链：激活冻结的角色更新分支并防同章双计、guest不再收编入池、autoCreateRoles开关接线生效、消费mergeCastRoleIds并归误建角色、名字归一与稳定名判定对齐上游、性别兜底回查角色档案）、`a99db1d`（AI分镜Helper加固：identityLinks别名链接生产落地、性别改判仅限explicit证据、同章生成互斥去重防双花AI调用、JSON提取与正文泄漏检查对齐上游、统一名字归一入口）、`e87f571`（分镜缓存与角色管理收尾：清空缓存改为按书生效并补确认文案、缓存列表按bookUrl过滤、转正命中已有角色时并入别名、删除正式角色解除临时角色链接、预生成章数读取钳制0..10）、`fcbce8d`（AI创作连线改为分区级：长按分区名发起连线、点击另一分区名完成，素材按分区组合并输出，卡片不再参与连线）、`c4e4a7d`（AI创作素材库：长按进入选择模式，底部弹出全选/删除栏，删除即同步摘除会话引用））。首次正式编译暴露源码错误 2 处（同一轮），修正后同版本通过——提交 `3abac75`：① `AiCreationLibraryDialog` 删除素材卡片误用裸 `session` 引用（未解析），改为 `AiCreationSessionHolder.session.removeCard`；② `AiConfigFragment` 打开智谱 API Key 官方首页 `openUrl` 缺 `Context` 接收者，补 `requireContext()`。最终一轮：BUILD SUCCESSFUL in 4m 11s（75 actionable tasks: 9 executed, 66 up-to-date），err.log 为空，退出码 0；构建期间无新提交。首轮 BUILD FAILED（`compileAppCKotlin` 报上述 2 处源码错误，43 tasks），失败包未产出、未交付。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082917c` / `10763`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.082917_10763.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10762` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`、无残留构建进程；未安装到模拟器、未做正式回归。
