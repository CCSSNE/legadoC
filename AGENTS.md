...
- `3.26.082414c` / `10731`，2026-08-24（UTC），`own` 主线（基于 `9191a09` 编译；版本号高于最近有效交付 `10728`，且高于无效的 `10729`/`10730` 构建）。用户要求先释放老编译程序再开始新编译：确认无残留 Gradle/Kotlin/Java 进程并 `gradlew --stop`（无 daemon）后，按本机内存压力规则冷编译 `:app:assembleAppC`（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`），后台 `.bat`（`build_10731.bat`，`Start-Process -WindowStyle Hidden`）stdout/stderr/退出码写入检出目录外 `D:\AI\audio\build-tmp`，退出码用 `(echo %EXITCODE%)>` 写法。编译前记录锚点 `own @ 9191a09`（工作区干净）；首轮启动后探测到另一会话并行重建 `build_10730.bat`（15:25:13 与 15:32:55 两轮），按“同一检出目录同时只允许一个构建、不得擅自杀他会话进程”规则主动终止己方首轮构建（taskkill 仅限己方 PID 树），待对方构建结束并清理其残留 Kotlin 编译器进程（~5.2 GB）后重新启动 `build_10731.bat`。BUILD SUCCESSFUL in 38s，退出码 0（先行构建已产出全部中间产物，`packageAppC` 重新打包携带新版本元数据）；构建期间无新提交，产物对应干净的 `9191a09`。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082414c` / `10731`、中文名 `阅读 C`（zh/zh-CN/zh-HK/zh-TW）、架构 `arm64-v8a`、compileSdk/targetSdk 36、`application-debuggable` 与 `10728` 基线一致；`apksigner` 退出码 0（META-INF 未受签名保护条目提示可接受）。产物文件名补 `c` 为 `legado_app_3.26.082414c_10731.apk`，收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`（对方后续构建可能清理产物目录）。因对方会话仍在构建 `10730`，为避免误停其单次 daemon 未执行 `gradlew --stop`（己方 `--no-daemon` 构建已自行退出，无己方残留进程）；未安装到模拟器、未做正式回归。注：`10729`（15:05–15:09 另一会话就地改 14 个源码文件，混合状态）与并发重建的 `10730` 均已判定无效未交付。
