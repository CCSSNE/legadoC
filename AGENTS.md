...
- `3.26.082921c` / `10764`，2026-08-29（UTC 21 时），`own` 主线（编译锚点 `own @ 439187b`，`own` 与 `origin/own` 同步；基于基线 `10763` 后新增提交 `5d15dac`（AGENTS.md 恢复 LF 行尾，消除整文件行尾噪声）、`aeb2f85`（AI创作第一页参数记忆：变量与模式写入即持久化，重开界面或重启应用均恢复上次值，清空才重置）、`4be3068`（正文长按菜单悬浮条宽度固定按窗口宽度计算，不再随菜单项变化反复改尺寸导致按钮跨行跳动）、`c3454f2`（普通日志模块归属钉定：双命中关键词的类在 LogModule 单点唯一归属，bdtts包/BdReadAloudService归百度TTS、TtsCacheLog归TTS缓存、TextFile内嵌JsExtensions归阅读）、`0705273`（AI创作预览新增一宫格并设为默认：一页一图独占整页，2/4宫格保留可切换）、`e847b4f`（AI创作变量定义对齐智谱官方参数：图片组 size七档带比例横竖标注/quality开关/水印开关，视频组按CogVideoX官方输出模式/AI音效/分辨率/帧率/时长；选项显示与取值分离，旧持久值自动回退默认；图片请求体接入 size/quality/watermark_enabled 并支持裸占位符）、`bcf0eb0`（修复百度TTS授权必败：bdTTSVerifyLicense 参数映射错误，licence.dat 路径误传冒充包名致 native fopen 失败恒返 ret=-8）、`0691547`（百度TTS发音人性别打标对齐AI分角色：编辑页新增性别选择 0=女/1=男/2=未标注，选音payload的gender改用 male/female/unknown，语音包缺失gender按未标注导入）、`a3d5c61`（百度TTS深度对照MultiTTS反编译源修正移植偏差：引擎初始化传路径串而非文件内容、补齐合成参数、修复音色编号静默失效）、`ed2145a`（日志新增AI分角色模块：分镜/收编/选音链路15处日志显式归属AI_CAST，不再误入朗读链路与百度TTS）、`439187b`（百度TTS 37111357 引擎处置：凭据完整还原但显式拒绝、修正错误结论））。最终一轮：BUILD SUCCESSFUL in 5m 14s（75 actionable tasks: 22 executed, 53 up-to-date），err.log 为空，退出码 0，无源码编译错误；构建期间无新提交。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082921c` / `10764`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.082921_10764.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10763` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`、无残留构建进程；未安装到模拟器、未做正式回归。
