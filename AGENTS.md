...
- `3.26.090201` / `10802`，2026-09-02（UTC 01 时），`own` 主线，**自用版 appC 编译交付**。本次为用户点名"编译自用版本"。`-Pabi=arm64-v8a -PVERSION_CODE=10802 -PVERSION_NAME=3.26.090201`，冷编译（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`，`GRADLE_OPTS=-Xmx4096m`）`assembleAppC` 首次失败：`compileAppCKotlin` 报 `Modifier 'override' is not applicable to 'local function'` / `Unresolved reference 'companion'` / `Missing '}'`，根因是 `dc3cfa2e`（离线评论体验修正）把 `launchOfflineReviewSend()` 包入 `lifecycleScope.launch` 时 `ReviewOutboxDispatcher.sendInBackground` lambda 丢失闭合 `}`，后续 `upScreenTimeOut`/`screenOffTimerStart`/`companion` 被误解析为局部函数；修复提交 `16f71ee7` 补上大括号并校正缩进。同版本重编译 BUILD SUCCESSFUL（4m37s）。`aapt` 确认包名 `io.legado.app.dev`、versionName `3.26.090201c`、versionCode `10802`、arm64-v8a；产物 `app\build\outputs\apk\app\c\legado_app_3.26.090201_10802.apk`（约 44.9 MB）。`apksigner` 退出码 0，证书 `CN=Android Debug`、SHA-256 `70cb88ca...`（debug 签名，符合签名铁律）。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。
