...
- `3.26.081609c` / `10621`，2026-08-15，已在雷电模拟器安装并截图回归。全局界面透明度 `0%` 时，`UiCorner.uiLayoutSurfaceAlpha()` 与 `floatingGroupAlpha()` 均为 `1`；主界面和辅助玻璃外壳统一以该物理 alpha 控制，外壳内部颜色保持不透明，因此书架顶部横向选择条和底部导航不再透出壁纸。书架封面透明度默认 `30%`，`0%` 不透明、`100%` 全透明，只控制书架普通书封面与书架合集拼图；“加入合集”弹窗中的合集预览继续使用弹窗透明度，但只由 `cover_mosaic` 整体合成一次，卡片底层透明、内部封面不再重复叠加透明度。原“书页背景透明”布尔开关及其偏好键、备份项、阅读页分支已移除，阅读页背景只使用自身背景透明度；弹窗模糊度摘要只显示百分比。弹窗透明度 `50%`、全局界面透明度 `0%` 保持独立。
