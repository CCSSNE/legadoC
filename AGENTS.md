...
- `3.26.083015c` / `10769`，2026-08-30（UTC 15 时），`own` 主线（编译锚点 `own @ 98cf0ae7`；基于基线 `10768`（锚点 `900ce9e5`）后新增提交 `252f5d90`（AI超时默认值调整：生成超时默认120改300秒，思考打断默认改为不打断）、`798c60cc`（修复AI流式思考打断异常竞态）、`d327e157`（普通日志弹窗新增导出菜单+LogExporter）、`f2c5c691`（新增未分类垃圾桶模块：classify未识别调用方归入未分类）、`512827af`（取消普通日志通用兜底：删除GENERAL，新增备份/视频/应用模块）、`45ed3fde`（TXT-ZIP导出支持附带TTS音频缓存）、`c8506299`（删除假复用的视频供应商复用图片开关）、`94adcc45`（AI创作配置体系重构：图片/视频拆成两套对称体系+提示词库改名字+纯文本）、`4b59bf82`（撤销日志弹窗导出菜单）、`4e02201e`（保存日志按钮改为保存勾选模块过滤后的普通日志）、`20624724`（视频生成补齐并发数量填写）、`ddecd08e`（AI选角新增拆分超长章节：拆分开关+章节字数上限）、`ecb80bb6`（本次编译修复：TTS音频缓存回导用ImportedArchiveBook.book访问书名与书Url且restore传Book、AI创作测试连接用promptLibrary收text）、`98cf0ae7`（本次编译修复：AiCreationImageFile多出一个多余右花括号导致顶层声明语法错误））。本次编译因锚点之后存在大量从未编译过的提交而首轮失败：`compileAppCKotlin` 报错——`LocalBook.kt:1031/1034/1036/1042`（`restoreTtsCacheFromArchive` 用 `importedBook.bookUrl`/`.name`，但 `ImportedArchiveBook` 只有 `entryName`/`book` 字段，且 `TtsCacheArchive.restore` 收 `Book`：改为 `importedBook.book.bookUrl`/`.book`/`.book.name`）、`AiConfigFragment.kt:474-475`（`AiCreationConfig.requestTemplates` 在重构后已删，现存 `promptLibrary(firstOrNull()?.text)`，旧 `.body` 改为 `.text`）、`AiCreationImageHelper.kt:156`（`saveVideoToAlbum` 为表达式函数，`.getOrDefault(false)` 后不应再补闭合括号；我多出的多余 `}` 导致 object 早闭、顶层声明语法错误）。修复提交 `ecb80bb6`/`98cf0ae7` 后重编译成功。最终一轮：BUILD SUCCESSFUL in 3m 20s（75 actionable tasks: 7 executed, 68 up-to-date），err.log 为空，退出码 0。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.083015c` / `10769`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.083015_10769.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10768` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`、无残留构建进程；未安装到模拟器、未做正式回归。
