...
- `3.26.082906c` / `10759`，2026-08-29（UTC 6 时），`own` 主线（编译锚点 `own @ 20d27c9`，`own` 与 `origin/own` 同步；基于基线 `10758` 后新增提交 `3b523c8`（百度离线TTS：BdReadAloudService + 引擎路由）、`4b9e587`（语音包导入器）、`bf7586d`（AI 创作）、`f20fcd4`、`b8a660c` 及文档提交 `c883c69`。编译暴露源码错误，共 6 轮修复后同版本通过）。修复轮次：① Kotlin 编译错误（AiCreationConfig 三引号字符串里 `\${参数}`/`\${素材}` 反斜杠不转义致 `${}` 被当作模板引用，改为 `${'$'}{...}`；补充缺失常量 `AI_CREATION_EPHEMERAL_BOOK`（`AiCreationConfig` 对象内 session 作用域卡片书籍名哨兵）；`AiCreationSession` 默认参数 `GROUP_IMAGE` 限定为 `AiCreationVariables.GROUP_IMAGE`；`ReadAloud` 重复 `BAIDU_ENGINE_ID`；`BdReadAloudService.play` 删除多余 `isRun = true`（`super.play()` 已设置）——提交 `e60d059`；② `AI_CREATION_EPHEMERAL_BOOK` 调用方 import 改为对象成员导入 `io.legado.app.help.ai.AiCreationConfig.AI_CREATION_EPHEMERAL_BOOK`——提交 `2478b47`，另补 Room schema 108 导出提交 `3e33237`；③ Java 阶段 `EmbeddedSynthesizerEnginf/Enginm` 引用不存在的 `IEmbeddedSynthesizerEnginf/Enginm`，改为实现现存的 `IEmbeddedSynthesizerEngine`——提交 `cdac1d1`；④ `BdEngineAdapter` 未初始化且未使用的 final 字段 `resPrefix` 删除、无抛出源的 `try/catch (InterruptedException)` 移除——提交 `25c1727`；⑤ 遗留 `try {` 与多余括号清理——提交 `20d27c9`。最终一轮：BUILD SUCCESSFUL in 36s（75 actionable tasks: 10 executed, 65 up-to-date），err.log 为空，退出码 0；构建期间无新提交。失败包未产出、未交付。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082906c` / `10759`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.082906_10759.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10758` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`（仅限本构建 Gradle home）、无残留构建进程；未安装到模拟器、未做正式回归。
