...
- `3.26.081618c` / `10634`，2026-08-16，已正式构建、APK 校验并安装到雷电模拟器。AI章节净化已接入阅读更多菜单，并从属于净化替换；AI 设置支持复用当前模型或保留可编辑的独立提供商/模型、提示词和处理参数。模拟器实测单独开启 AI 会带起净化替换，关闭净化替换会同时关闭 AI；章节加载的缺模型失败记录为单次，未出现重复调度或崩溃。
