...
- 最近一次自用版交付为 `3.26.090913c` / `10883`，2026-09-09，基于提交 `9a570cdf`（自播媒体缓存占用存储时允许删除，不再要求完整章节计数）使用 `assembleAppC` 编译成功（`BUILD SUCCESSFUL in 49s`，`75 actionable tasks: 13 executed, 62 up-to-date`）。产物包名 `io.legado.app.dev`、versionName `3.26.090913c`、versionCode `10883`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner verify` 退出码 0；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090913_10883.apk`，尚未安装。
