...
- 最近一次自用版交付为 `3.26.090507` / `10845`，2026-09-05，基于提交 `9c00fb81`（自 `10844` 基线后 7 个提交：LLM请求体拆分独立、爆炸补分镜选角净化、创作收回全局、供应商多模态开关、标记规则进库、语义位置、去否定句）使用 `assembleAppC` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，`BUILD SUCCESSFUL in 4m 4s`，err 为空；拉起工具调用报 `ChildProcess.kill` 超时，启动器 PID `24496` 存活、日志持续增长，属已知现象，未重复拉起）。注意：本机 08:34 另有一轮同为 `10845` 的构建先完成（`BUILD SUCCESSFUL in 4m 49s`，发起方不明），本轮 08:44 启动时其已结束、无目录锁冲突；本轮重跑了 `ksp/compileAppCKotlin/dex/package` 全链路，APK 时间戳 08:48:58 为本轮产物。产物包名 `io.legado.app.dev`、versionName `3.26.090507c`、versionCode `10845`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C-自用`（label-zh 逐字匹配）、`apksigner` 验证通过（退出码 0，`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\app\c\legado_app_3.26.090507_10845.apk`（50467147 字节）。上一自用版为 `3.26.090506` / `10844`；最近公开版为 `3.26.090420` / `10837`，2026-09-04，基于提交 `4a69c948`（README 更新 2026-09-04 记录）使用 `assembleOssRelease` 冷编译成功（`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Pkotlin.compiler.execution.strategy=in-process`，计划任务脱离宿主进程树拉起，`BUILD SUCCESSFUL in 7m 15s`，err 为空；任务/启动器事后已删）。产物包名 `io.legado.app.c`、versionName `3.26.090420`（无后缀）、versionCode `10837`、架构 `arm64-v8a`，`aapt` 确认应用名 `阅读C`（label-zh 逐字匹配）、`apksigner` 验证通过（退出码 0，`META-INF` 未保护条目提示可接受）；APK 位于 `app\build\outputs\apk\oss\release\legado_oss_3.26.090420_10837.apk`（34592960 字节），已发布 Release `v3.26.090420`（正式版 draft=false / prerelease=false，资产下载 200）。此前同日编译过 `10836`（versionName 同为 `3.26.090420`，基于 `f4c88c5a`，未发布 Release，产物已被 10837 覆盖删除）。上一公开版为 `3.26.090400` / `10827`（曾创建 Pre 版 Release `v3.26.090400`）。
