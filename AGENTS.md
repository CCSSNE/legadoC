...
- `3.26.083107` / `10785`，2026-08-31（UTC 07 时），`own` 主线，**双编译交付**（自用版 appC + 公开版 oss，编译锚点 `own @ c31eae97`；基于基线 `10784`（锚点 `bf72042b`）后新增提交 `6aff8efb`（包名身份重映射：公开版统一 `io.legado.app.c`、自用版独立 `io.legado.app.dev`）、`245b2710→f6eb3a2f`（签名策略经"强制正式 myConfig"最终定稿为"只允许 release 系构建、统一 debug 签名"）、`c31eae97`（修复 appC 编译：为 `BdTtsCacheSynthesizer` 补充缺失 import——作者提交 `e51ad48fe` 引入该引用但漏导包，oss 剥离 `app/src/app` 源集致漏检））。两路均 `-Pabi=arm64-v8a -PVERSION_CODE=10785 -PVERSION_NAME=3.26.083107`，冷编译 `--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`。**自用版** `assembleAppC` BUILD SUCCESSFUL in 2m 51s（75 actionable tasks: 7 executed, 68 up-to-date），`aapt` 确认包名 `io.legado.app.dev`、versionName `3.26.083107c`、arm64-v8a、应用名 `legado C`；产物 `app\build\outputs\apk\app\c\legado_app_3.26.083107_10785.apk`（46.07 MB），备份于 `D:\AI\audio\build-tmp\legado_app_3.26.083107_10785.apk`。**公开版** `assembleOssRelease` BUILD SUCCESSFUL in 5m 8s（121 actionable tasks: 23 executed, 98 up-to-date），`aapt` 确认包名 `io.legado.app.c`、versionName `3.26.083107`（无后缀）、arm64-v8a、应用名"阅读"；产物 `app\build\outputs\apk\oss\release\legado_oss_3.26.083107_10785.apk`（33.07 MB），备份于 `D:\AI\audio\build-tmp\legado_oss_3.26.083107_10785.apk`。两包 `apksigner` 退出码均 0，证书均 `CN=Android Debug`、SHA-256 `70cb88ca...`（同一把 debug 签名，符合只允许 release 系构建 + 统一 debug 签名的签名铁律）。两包包名不同可共存于同一设备，公开版同签名同包名按 versionCode 递增可正常覆盖。旧 10783/10784 产物（`legado_oss_3.26.083106_10784.apk` refgd 旧包名已作废）已从输出目录清理。构建后 `gradlew --stop` 确认无残留 Gradle 进程；未安装到模拟器、未做正式回归。
