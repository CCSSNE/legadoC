...
- `3.26.082413c` / `10729`，2026-08-24（UTC），`own` 主线（基于 `90220df` 编译；版本号在先前交付 `10728` 之上单调递增）。持续监控模式下新提交 `faa178f`（缓存页下载路由到媒体）、`90220df`（沉浸菜单加离线缓存）到达后触发编译。首次编译在 `compileAppCKotlin` 阶段失败：daemon 日志显示 Kotlin 编译器走 daemon 通道时 `releaseCompileSession` 报 `Connection refused` / “Unable to release compile session, maybe daemon is already down”，无 `hs_err`、内存充足（31.7GB 总量余 8.6GB）、err 流为空；非源码错误，按 daemon 类故障流程收尾（`gradlew --stop`、确认无残留 java/Gradle 进程、确认另一会话脚本 `assemble_10729.bat` 未启动）后以相同冷编译参数重试成功（期间分支无新提交，HEAD 保持 `90220df`）。命令：`--no-daemon --max-workers=1 -Dkotlin.incremental=false -Dksp.incremental=false -Dkotlin.compiler.execution.strategy=in-process`，后台 `.bat` 隐藏窗口启动，stdout/stderr/退出码写入 `D:\AI\audio\build-tmp`。BUILD SUCCESSFUL in 4m 49s，退出码 0（out 流含 `w: Detected multiple Kotlin daemon sessions` 警告，非失败）。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082413c` / `10729`、中文名 `阅读 C`（zh，含 zh-CN/zh-HK/zh-TW）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（META-INF 未受签名保护条目提示可接受）。产物文件名原为 `legado_app_3.26.082413_10729.apk`，内部元数据正确，仅改名补 `c` 为 `legado_app_3.26.082413c_10729.apk`；旧 `10728` APK 已从产物目录移除。收尾 `gradlew --stop`，无残留 java/Gradle/kotlin 进程。持续监控仍在运行：下一锚点为 `90220df`（本轮编译 HEAD），新提交出现将基于新 HEAD 重编译（下一轮起 `VERSION_CODE` 递增为 `10730` 起）；未安装到模拟器、未做正式回归。
