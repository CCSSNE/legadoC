...
- `3.26.082400c` / `10719`，2026-08-24（UTC），`own` 主线（基于 `7137d64` 编译：fix(review) add missing AppConfig import；版本号在先前交付 `10718` 之上单调递增）。用户明确要求正式编译。编译前按用户选择先把未提交的「重构下载架构.md 移入 docs/」作为独立文档提交（`40610f0`）并推送，再从干净 HEAD 启动编译。首次编译在 `compileAppCKotlin` 失败：`ReviewSnapshotManager.kt:305 Unresolved reference 'AppConfig'`——上一提交新增开关检查时遗漏 import；补上 `import io.legado.app.help.config.AppConfig` 并提交（`7137d64`）后以同一 versionCode/versionName 重编译成功（BUILD SUCCESSFUL in 4m 16s，退出码 0）。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.082400c` / `10719`、中文名 `阅读 C`（zh locale）、架构 `arm64-v8a`、compileSdk/targetSdk 36；`apksigner` 完整执行退出码 0（仅 META-INF 未签名条目警告，可接受）。产物文件名原为 `legado_app_3.26.082400_10719.apk`，内部元数据正确，按规则仅修正文件名末尾的 `c`（现为 `legado_app_3.26.082400c.apk`），未为此重编译。构建期间分支无新提交（HEAD 保持 `7137d64`），无需重编。收尾无残留 java 进程，守护进程已停。基线记录提交正常推送；未安装到模拟器、未做正式回归。
