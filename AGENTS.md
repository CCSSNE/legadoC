...
- `3.26.083001c` / `10767`，2026-08-30（UTC 01 时），`own` 主线（编译锚点 `own @ 2821402b`；基于基线 `10766` 后新增提交 `49956ca2`（AI生图连线组改组制显示：连线组命名A组/B组并随成员加入或并组保持稳定，分区名旁以组名取代笼统已连线，长按已连线分区新增发起连线选项）、`55012305`（修复引号多角色无效：bdMultiVoice隐藏开关无UI入口致分段永不生效，改为配置旁白池/对白池即启用，对白与旁白按引号分开随机路由，与AI音色开关解耦）、`2eaa47cb`（拖动朗读进度条等效回原进度：seek命令携带syncView显式传送标记，位置发布透传，显示侧复用原语B对齐，含回退与暂停态）、`48039a20`（AI 创作新增图片/视频供应商管理：内置硅基流动与智谱，测试连接真实请求，删除4个裸图片设置）、`b2587c76`（新增Git钩子强制中文提交与防直推公开仓：commit-msg校验中文、pre-push拦截own直推origin/own）、`3147062f`（百度引擎管理页新增语音包导出与发音人删除：导出从存储反向生成config.yaml并原样打包数据树，编辑发音人对话框新增红色删除按钮）、`1360c740`（本次编译修复：parseCustomHeaders未限定对象调用）、`2821402b`（本次编译修复：图片测试连接归属对象修正））。本次编译首轮失败：`compileAppCKotlin` 报 `AiChatService.kt:671/903` `Unresolved reference 'parseCustomHeaders'`——该函数实为 `AiCreationProviderStore` 对象成员，两处漏限定对象调用；同时报 `AiConfigFragment.kt:26/978` `Unresolved reference 'AiCreationImageHelper'`——该文件实际声明 `AiCreationImageFile`（文件管理）与 `AiCreationImageTaskHolder`（任务与 `testConnection`）两个对象，不存在 `AiCreationImageHelper` 类名，测试连接应指向 `AiCreationImageTaskHolder.testConnection`。修复提交 `1360c740`/`2821402b` 后重编译成功。最终一轮：BUILD SUCCESSFUL in 4m 31s（75 actionable tasks: 7 executed, 68 up-to-date），err.log 为空，退出码 0。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.083001c` / `10767`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.083001_10767.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10766` APK 已由 Gradle 重建产物目录自动清除。构建后 `gradlew --stop`、无残留构建进程；未安装到模拟器、未做正式回归。
