...
- `3.26.081741c` / `10662`，2026-08-17（GMT+8），`feature/audio-reading-ui-unification` 分支。根因修复：重构把所有书的朗读强制指向未注册且尚未实现完整文本朗读语义的 `UnifiedReadAloudService`，导致普通本地 TXT 点击“朗读”后系统报 `Unable to start service ... not found`；`ReadAloud.getReadAloudClass()` 现统一按书籍与引擎能力选择服务，音频书进入融合服务，普通文本书继续使用成熟的 `TTSReadAloudService` / `HttpReadAloudService`，并在 Manifest 正式注册融合服务（代码提交 `98ba8a8`）。已通过隐藏后台正式 `:app:assembleAppC` 构建，退出码为 0；`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.081741c` / `10662`、中文名“阅读 C”、架构 `arm64-v8a`，`apksigner` 退出码为 0。已覆盖安装到雷电模拟器 `emulator-5554`，使用书架中的真实本地 TXT 完整回归：阅读页主菜单点击“朗读”后实际启动 `TTSReadAloudService`，朗读面板显示，媒体会话为 `PLAYING`，面板“停止”可结束服务，复现窗口无 FATAL 且不再出现服务找不到。已确认但尚未修复的下一问题：新进度条显示“段 27 / 段 0”，总段落数数据源错误，应作为独立修改继续定位，禁止用 UI 默认值掩盖。
