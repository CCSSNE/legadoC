...
- `3.26.083020c` / `10772`，2026-08-30（UTC 20 时），`own` 主线（编译锚点 `own @ 6e3d632c`；基于基线 `10771`（锚点 `623735b8`）后新增提交 `6e3d632c`（修复V2脚本引擎候选音色gender可空导致的编译错误：未知/缺失性别归一到unknown））。本次编译首次失败：`compileAppCKotlin` 报 `BookTtsCastingCoordinator.kt:622 Argument type mismatch: actual type is 'String?', but 'String' was expected`——`TtsVoice.gender`（可空）直接赋给 `TtsVoiceInfo.gender`（非空，约定 male/female/unknown）。修复：`voice.gender?.takeIf { it == BookRole.Gender.MALE || it == BookRole.Gender.FEMALE } ?: BookRole.Gender.UNKNOWN`。修复后同版本 10772 重编译成功，BUILD SUCCESSFUL in 5m 4s（75 actionable tasks: 7 executed, 68 up-to-date），err.log 仅含 Kotlin 弃用 `w:` 警告、无错误，退出码 0。`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.083020c` / `10772`、中文名 `阅读 C`（zh）、架构 `arm64-v8a`、compileSdk 36；`apksigner` 完整执行退出码 0（Signer #1 CN=Android Debug，SHA-256 `70cb88ca...`）。产物 `legado_app_3.26.083020_10772.apk`（Gradle 自动命名）收录于 `app\build\outputs\apk\app\c` 并备份至 `D:\AI\audio\build-tmp`，旧 `10771` APK 已删除。构建后 `gradlew --stop`、按 PID 清理残留 Gradle/Kotlin daemon；未安装到模拟器、未做正式回归。
