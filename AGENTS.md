...
- `3.26.081742c` / `10663`，2026-08-17（GMT+8），`feature/audio-reading-ui-unification` 分支。完成本地文本书朗读体验修复（代码提交 `75fe52a`）：删除朗读面板硬编码的 `0 / 0` 与仅改标签的伪拖动，建立带章节、位置、总量和单位的 `ReadAloudProgress` 统一状态；成熟 `BaseReadAloudService` 从实际 `contentList` 发布段落进度，并使用 `TextChapter.getParagraphs()` 的排版后 `chapterPosition` 执行段落定位，过期章节命令会被拒绝，非法映射直接记录并停止朗读；悬浮封面入口恢复为阅读页内打开控制面板、阅读页外通过 `startActivityForBook()` 回到普通阅读页，不再错误进入只支持音频书的 `AudioPlayActivity`。已通过隐藏后台正式 `:app:assembleAppC` 构建，退出码为 0；`aapt` 确认包名 `io.legado.app.c`、版本 `3.26.081742c` / `10663`、中文名“阅读 C”、架构 `arm64-v8a`，`apksigner` 退出码为 0。已覆盖安装到雷电模拟器 `emulator-5554`，使用书架真实本地 TXT 回归：主菜单启动的是 `TTSReadAloudService`；面板显示真实 `段 22 / 段 32` 和 `段 7 / 段 80`；暂停后拖至 `段 62 / 段 80`，服务收到 `chapterIndex=1, paragraphIndex=61`，保持 `PAUSED`，恢复日志确认从 `nowSpeak=61, pageIndex=6` 续播；关闭重开面板仍保留真实进度；显示悬浮窗后，阅读页点击封面打开面板，播放状态下通过“后台”回到书架后点击封面重新进入 `ReadBookActivity`，全过程未启动 `AudioPlayActivity`、无 FATAL。测试后已恢复“隐藏朗读悬浮窗”原值并停止服务。在线书源、音频书源播放与全屏听书页未纳入本次本地文本书交付结论。
